...
`validate.mjs` 不安装或加载第三方依赖，当前固定 `PHASE0_2026_08_13_IMPORT_SAFETY_PREFLIGHT_CONTRACT` 基线并检查：
...
- 每日事件文件、日期前缀、连续序号、记录日期和版本化的 `59/13/5/29/5/15/5` 日分布。历史事件存在已知的时间回填逆序，因此不以物理行时间单调作为失败条件。
...
`reconcile.mjs` 是只读诊断器：它重新从事件、消息、决定台账、Owner intake、证据矩阵和人工快照读取数据，报告源计数、快照指标、最新源时间、已记录的 OI-03 设备事实、Owner 原生 `OI-02` choice-ui 门禁，以及 D-039/D-040 当前授权位。它不会覆盖 `snapshots/current.json`；当前人工快照已同步至 2026-08-13 的 F19 导入预检合同登记事件，并与最新来源时间一致。
