...
`validate.mjs` 不安装或加载第三方依赖，当前固定 `PHASE0_2026_08_15_D039_PX5_DOR_ASSESSMENT` 基线并检查：

- 5 份 Draft 2020-12 Schema 定义与受控映射；使用仓库内 `DRAFT_2020_12_PROJECT_SUBSET_V1` profile 校验 `decisions.json`、`owner-intake.json`、`snapshots/current.json`、全部 Event 和 Message，当前共 282 个实例。Schema 负责结构与类型，精确计数、Gate、Agent 和跨源事实仍由版本化运营不变量负责。
...
- D-039 保留历史 `PX-2_PASS` 与 Owner `A / ACCEPTED / PX-3_PASS`，并精确冻结 PX-4 首层层级和四域复核；首次 PX-5 DoR 评估固定为 `NOT_READY / 7 BLOCKERS / PX-5_BLOCKER_CLOSURE_REQUIRED`，正式实现、正式根工程、原生 iOS、D-032 第二次动作和 D-053 授权均保持关闭。
...
内置 profile 只支持当前 5 份 Schema 实际使用的 `$defs`、本地 `$ref`、type/const/enum、required/properties/items/additionalProperties、字符串/数组约束、pattern、RFC 3339 date/date-time 等关键字。新增未支持关键字、外部或循环 `$ref` 会失败关闭，不能把本工具解释为通用 JSON Schema 引擎。完整 Draft 2020-12 元 Schema 合规仍须由 AJV 8 + `ajv-formats` 或后续经批准的等价 validator 交叉检查；当前 PASS 精确表示“Schema 定义符合仓库 profile 且 282 个受控实例通过校验”。

`reconcile.mjs` 是只读诊断器：它重新从事件、消息、决定台账、Owner intake、证据矩阵和人工快照读取数据，报告源计数、快照指标、最新源时间、已确认的 OI-02 标识状态与 OI-03 设备事实、D-032 隔离 JS Spike 授权、D-039=A 接受/PX-4/PX-5 NOT_READY、D-040 的 20 轴分解、候选 ID 预留、第一批四卡自审与计划中的 D-040 Owner 卡，以及 D-039/D-040 当前授权位。它不会覆盖 `snapshots/current.json`；当前人工快照已同步至 2026-08-15 D-039 PX-5 DoR 评估事件并与最新来源时间一致。
