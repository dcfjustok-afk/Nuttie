...
const CONTRACT_ID = "D039-B03-B05-REVIEWER-ASSIGNMENT-CONTRACT-001";
const INPUT_SCHEMA_VERSION = "D039_B03_B05_REVIEWER_ASSIGNMENT_INPUT_V1";
const RESULT_SCHEMA_VERSION = "D039_B03_B05_REVIEWER_ASSIGNMENT_RESULT_V1";
const BOUNDARY_SCHEMA_VERSION = "D039_B03_B05_REVIEWER_ASSIGNMENT_BOUNDARY_V1";
const INTAKE_PACKET_ID = "D039-B03-B05-REVIEWER-INTAKE-PACKET-001";
...
function fail(message, code = "INVALID_D039_REVIEWER_ASSIGNMENT", details = {}) {
...
function assertSafeFieldName(key, field) {
  ...
  if (SENSITIVE_FIELD_NAMES.has(normalized)) {
    fail("input contains a prohibited sensitive field", "UNSAFE_D039_REVIEWER_ASSIGNMENT", { field });
  }
}

function assertSafeString(value, field) {
  if (containsSensitiveLookingValue(value)) {
    fail("input contains prohibited sensitive-looking material", "UNSAFE_D039_REVIEWER_ASSIGNMENT", { field });
  }
}
...
function assertPersonName(value, field, recordKind) {
  ...
  const roleOnly = new Set([
    ...
    "product decision quality", "privacy data integrity", "security transport resource evidence",
    "qa accessibility",
  ]);
...
function normalizeCandidate(value, index, recordKind) {
  ...
  const candidatePattern = recordKind === SYNTHETIC_RECORD_KIND
    ? /^D039-B03-B05-SYNTHETIC-C\d{3}$/
    : /^D039-B03-B05-REVIEWER-C\d{3}$/;
...
function normalizeD039ReviewerAssignment(input) {
  ...
  const assignmentPattern = input.recordKind === SYNTHETIC_RECORD_KIND
    ? /^D039-B03-B05-SYNTHETIC-A\d{3}$/
    : /^D039-B03-B05-ASSIGNMENT-A\d{3}$/;
  ...
  for (const flag of ["containsCredential", "containsIdentityDocument", "containsPrivateContact", "containsSignatureMaterial"]) {
    if (input[flag] !== false) fail(`${flag} must be false`, "UNSAFE_D039_REVIEWER_ASSIGNMENT", { field: flag });
  }
...
function evaluateD039ReviewerAssignment(input) {
...
function validateD039ReviewerAssignmentResult(result, input) {
  assertDataTree(result, "result");
  const expected = evaluateD039ReviewerAssignment(input);
