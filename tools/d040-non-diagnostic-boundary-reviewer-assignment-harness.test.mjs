...
import {
  ...
  evaluateD040NonDiagnosticBoundaryReviewerAssignment,
  normalizeD040NonDiagnosticBoundaryReviewerAssignment,
  validateD040NonDiagnosticBoundaryReviewerAssignmentResult,
} from "./d040-non-diagnostic-boundary-reviewer-assignment-harness.mjs";
...
function makeSyntheticFixture() {
  const reviewer = {
    candidateId: "D040-NDB-SYNTHETIC-C001",
  ...
  return withDigest({
    ...
    assignmentId: "D040-NDB-SYNTHETIC-A001",
...
function makeFormalFixture() {
  ...
  input.assignmentId = "D040-NDB-ASSIGNMENT-A001";
  const reviewer = input.reviewers[0];
  reviewer.candidateId = "D040-NDB-REVIEWER-C001";
...
test("完整合成四域指派只覆盖算法且现实授权全部保持关闭", () => {
  const input = makeSyntheticFixture();
  const result = evaluateD040NonDiagnosticBoundaryReviewerAssignment(input);
...
test("结构完整正式输入只返回调用方声明的 candidate 而不形成现实指派", () => {
  const result = evaluateD040NonDiagnosticBoundaryReviewerAssignment(makeFormalFixture());
  ...
  assert.deepEqual(result.blockers, [
    ...
    "D040_NDB_HEALTH_OWNER_PX_AND_IMPLEMENTATION_GATES_UNCHANGED",
  ]);
  for (const field of [
    ...
    "independentReviewStarted", "nonDiagnosticBoundaryIndependentReviewPassed",
    "d068OwnerReady", "d069OwnerReady",
    "healthReviewerAssigned", "healthContentApproved", "contentQaPassed",
    ...
    "diagnosisOrTreatmentAuthorized", "medicationDetailCollectionAuthorized", "healthFreeTextCollectionAuthorized",
    "healthDataPersistenceAuthorized", "automaticDialAuthorized", "networkResourceRefreshAuthorized",
    "locationReadAuthorized", "contactsReadAuthorized", "healthKitWriteAuthorized",
    "formulaImplementationAuthorized", "healthCopyImplementationAuthorized",
...
test("正式记录拒绝合成 identity 与 example.test 引用", () => {
  const input = mutateFixture((draft) => {
    ...
    draft.assignmentId = "D040-NDB-ASSIGNMENT-A001";
    draft.reviewers[0].candidateId = "D040-NDB-REVIEWER-C001";
    for (const coverage of draft.domainCoverage) coverage.candidateIds = ["D040-NDB-REVIEWER-C001"];
  });
  assert.throws(
    () => normalizeD040NonDiagnosticBoundaryReviewerAssignment(input),
...
test("顶层字段缺失、额外字段和 record kind 漂移时失败关闭", () => {
  assert.throws(() => normalizeD040NonDiagnosticBoundaryReviewerAssignment(mutateFixture((draft) => {
...
test("冻结 packet 身份任一漂移都被拒绝", () => {
  for (const [field, value] of [
    ...
    ["packetEventId", "EVT-20260821-999"],
    ["cardSpecEventId", "EVT-20260827-999"],
    ["cardHarnessEventId", "EVT-20260827-998"],
    ["reviewRecordHarnessEventId", "EVT-20260827-997"],
    ...
    assert.throws(() => normalizeD040NonDiagnosticBoundaryReviewerAssignment(input), /frozen packet/);
  }
});
...
test("candidate ID、角色名和重复身份不能冒充具名复核人", () => {
  ...
  assert.throws(() => normalizeD040NonDiagnosticBoundaryReviewerAssignment(duplicate), /duplicate candidate IDs/);
});
...
test("四域覆盖矩阵拒绝顺序漂移、未知 candidate 和单向声明", () => {
  ...
  assert.throws(() => normalizeD040NonDiagnosticBoundaryReviewerAssignment(mutateFixture((draft) => {
    draft.domainCoverage[2].candidateIds = ["D040-NDB-SYNTHETIC-C999"];
...
test("未授权联络或缺少 assignment evidence 只能保持 incomplete", () => {
  ...
  const unauthorizedResult = evaluateD040NonDiagnosticBoundaryReviewerAssignment(unauthorized);
  ...
  const noEvidenceResult = evaluateD040NonDiagnosticBoundaryReviewerAssignment(noEvidence);
...
test("packet 接受、指派、截止时间与签署计划失败关闭", () => {
  ...
  const lateResult = evaluateD040NonDiagnosticBoundaryReviewerAssignment(lateAssignment);
...
test("reviewCanStart 必须由 validator 重算，调用方不能伪造", () => {
  ...
  assert.throws(() => normalizeD040NonDiagnosticBoundaryReviewerAssignment(forged), /does not match recomputed/);
});
...
test("assignmentContentSha256 绑定删除自身后的完整 bundle", () => {
  ...
  assert.throws(() => normalizeD040NonDiagnosticBoundaryReviewerAssignment(input), /does not match the canonical bundle/);
  const malformed = makeSyntheticFixture();
  malformed.assignmentContentSha256 = "A".repeat(64);
  assert.throws(() => normalizeD040NonDiagnosticBoundaryReviewerAssignment(malformed), /lowercase SHA-256/);
});
...
test("敏感字段名和值在解析前拒绝且错误不回显原值", () => {
  ...
  const fieldError = captureError(() => normalizeD040NonDiagnosticBoundaryReviewerAssignment(emailInput));
  assert.equal(fieldError.code, "UNSAFE_D040_NDB_REVIEWER_ASSIGNMENT");
  ...
  const valueError = captureError(() => normalizeD040NonDiagnosticBoundaryReviewerAssignment(valueInput));
  assert.equal(valueError.code, "UNSAFE_D040_NDB_REVIEWER_ASSIGNMENT");
  ...
  const phoneError = captureError(() => normalizeD040NonDiagnosticBoundaryReviewerAssignment(phoneInput));
  assert.equal(phoneError.code, "UNSAFE_D040_NDB_REVIEWER_ASSIGNMENT");
  assert.doesNotMatch(phoneError.message, /13812345678/);

  assert.throws(() => normalizeD040NonDiagnosticBoundaryReviewerAssignment(mutateFixture((draft) => {
    draft.containsIdentityDocument = true;
  })), (error) => error.code === "UNSAFE_D040_NDB_REVIEWER_ASSIGNMENT");
});

test("特殊对象、accessor、cycle、超长字符串和 reviewer 上限都被拒绝", () => {
  const mapInput = makeSyntheticFixture();
  mapInput.reviewPacketIdentity = new Map();
  assert.throws(() => normalizeD040NonDiagnosticBoundaryReviewerAssignment(mapInput), /plain records/);
  ...
  assert.throws(() => normalizeD040NonDiagnosticBoundaryReviewerAssignment(accessorInput), /non-data properties/);

  const cycleInput = makeSyntheticFixture();
  cycleInput.loop = cycleInput;
  assert.throws(() => normalizeD040NonDiagnosticBoundaryReviewerAssignment(cycleInput), /cycle/);

  const longInput = makeSyntheticFixture();
  longInput.reviewers[0].controlledContactRef = `urn:nuttie.example.test:${"x".repeat(600)}`;
  assert.throws(() => normalizeD040NonDiagnosticBoundaryReviewerAssignment(longInput), /resource limits/);

  const crowded = makeSyntheticFixture();
  crowded.reviewers = Array.from({ length: 21 }, () => clone(crowded.reviewers[0]));
  assert.throws(() => normalizeD040NonDiagnosticBoundaryReviewerAssignment(crowded), /one to twenty candidates/);
});

test("规范化与结果深冻结，伪造结果或指纹无法通过重建", () => {
  const input = makeSyntheticFixture();
  const normalized = normalizeD040NonDiagnosticBoundaryReviewerAssignment(input);
  ...
  assert.deepEqual(validateD040NonDiagnosticBoundaryReviewerAssignmentResult(result, input), result);
  ...
  assert.throws(() => validateD040NonDiagnosticBoundaryReviewerAssignmentResult(forged, input), /changed/);
});

test("源码审计固定无 Git、文件、联系人、签署、网络、消息或业务副作用", () => {
  const sourcePath = fileURLToPath(new URL("./d040-non-diagnostic-boundary-reviewer-assignment-harness.mjs", import.meta.url));
