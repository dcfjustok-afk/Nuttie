...
const INPUT_SCHEMA_VERSION = "D040_FIRST_THREE_BATCHES_INDEPENDENT_REVIEW_BUNDLE_INPUT_V1";
const RESULT_SCHEMA_VERSION = "D040_FIRST_THREE_BATCHES_INDEPENDENT_REVIEW_RESULT_V1";
const BOUNDARY_SCHEMA_VERSION = "D040_FIRST_THREE_BATCHES_INDEPENDENT_REVIEW_BOUNDARY_V1";
const CONTRACT_ID = "D040-FIRST-THREE-BATCHES-INDEPENDENT-REVIEW-RECORD-CONTRACT-001";
...
const REVIEWED_ARTIFACTS = Object.freeze([
  Object.freeze({ order: 1, path: "docs/03-design/d040-question-allocation.md", gitBlobOid: "380435e7bbe611319b102052662da471ff9e49b1", sha256: "20fd5c4671419c17ab06d99a8f1241169b67b8009b86b51fc1181fb66847ce08" }),
  Object.freeze({ order: 2, path: "docs/03-design/d040-px0-input-research.md", gitBlobOid: "f3b9e68d4b181b761e21a57ba476291d7410cf36", sha256: "bf7b4c6e74307b93a15c38c47cf3c81a3c5b45e651fcb4b1b3a02ef9b2a51381" }),
  Object.freeze({ order: 3, path: "docs/03-design/d040-first-batch-card-spec.md", gitBlobOid: "c55e5d73a8cffc31ee81fb9d72dd2c252ea08282", sha256: "8489e99efbdb2f2f410eb1005909dd2b1732d8a8ce69616aca6eec51f8d86ef9" }),
  Object.freeze({ order: 4, path: "docs/03-design/d040-energy-model-batch-card-spec.md", gitBlobOid: "46f3a6b353ebfa9c2ab73f76b291873dbd9f6569", sha256: "e776e8f7ca9aa9649849ef2b6cc814e6e0c461c8b55e7f0f0f6ae4e517373835" }),
  Object.freeze({ order: 5, path: "docs/03-design/d040-niddk-dynamic-model-feasibility-input.md", gitBlobOid: "409119ac4af1691791794a733364d50f847653b2", sha256: "6feeba9bf07991c66254cf42250eefdf5d082de155417d2c7490a59a679b00b0" }),
  Object.freeze({ order: 6, path: "docs/03-design/d040-data-lifecycle-batch-card-spec.md", gitBlobOid: "cbf152542e9c5d6020e311dd2e859e89a7aa3881", sha256: "55cd099d3dad3ddd8244a46e1c78d0d4d31f5426af9b53af73b1f9bf3378a567" }),
  Object.freeze({ order: 7, path: "docs/03-design/d040-china-health-reviewer-intake-packet.md", gitBlobOid: "89f66cb38da0cd2865a343ac471e1cbe63de92c8", sha256: "7e48fa29be626429b63c31492d37b710f8f873d5f079aeb5c70dee918bf5f110" }),
]);

const CARD_IDENTITIES = Object.freeze([
  Object.freeze({ batchId: "BATCH_1", decisionId: "D-054", questionId: "d054_formula_age_scope" }),
  Object.freeze({ batchId: "BATCH_1", decisionId: "D-055", questionId: "d055_age_source_retention" }),
  Object.freeze({ batchId: "BATCH_1", decisionId: "D-056", questionId: "d056_formula_age_representation" }),
  Object.freeze({ batchId: "BATCH_1", decisionId: "D-058", questionId: "d058_formula_branch_policy" }),
  Object.freeze({ batchId: "BATCH_2", decisionId: "D-057", questionId: "d057_base_energy_path" }),
  Object.freeze({ batchId: "BATCH_2", decisionId: "D-059", questionId: "d059_activity_input_representation" }),
  Object.freeze({ batchId: "BATCH_2", decisionId: "D-060", questionId: "d060_missing_activity_behavior" }),
  Object.freeze({ batchId: "BATCH_2", decisionId: "D-061", questionId: "d061_mifflin_ree_use" }),
  Object.freeze({ batchId: "BATCH_2", decisionId: "D-062", questionId: "d062_weight_change_goal_path" }),
  Object.freeze({ batchId: "BATCH_3", decisionId: "D-064", questionId: "d064_profile_goal_storage" }),
  Object.freeze({ batchId: "BATCH_3", decisionId: "D-065", questionId: "d065_profile_deletion_semantics" }),
  Object.freeze({ batchId: "BATCH_3", decisionId: "D-066", questionId: "d066_energy_display_rounding" }),
  Object.freeze({ batchId: "BATCH_3", decisionId: "D-067", questionId: "d067_recalculation_policy" }),
]);
const INVARIANT_IDS = Object.freeze(Array.from({ length: 12 }, (_, index) => `D040-FTB-XCI-${String(index + 1).padStart(3, "0")}`));
...
function fail(message, code = "INVALID_D040_FIRST_THREE_BATCHES_INDEPENDENT_REVIEW_RECORD", details = {}) {
...
function normalizeString(value, field, { maximum = 512, minimum = 1 } = {}) {
  ...
  if (containsSensitiveLookingMaterial(normalized)) {
    fail("input contains prohibited sensitive-looking material", "UNSAFE_D040_FIRST_THREE_BATCHES_INDEPENDENT_REVIEW_RECORD", { field });
  }
  return normalized;
}
...
function normalizeReviewedArtifacts(value) {
  if (!Array.isArray(value) || value.length !== REVIEWED_ARTIFACTS.length) fail("reviewedArtifacts must contain exactly seven entries", undefined, { field: "reviewedArtifacts" });
...
function normalizeFindingIds(value, field, minimum = 0) {
  ...
  const normalized = value.map((item, index) => {
    ...
    if (!/^D040-FTB-IR-F\d{3}$/.test(findingId)) fail(`${field}[${index}] must be D040-FTB-IR-FNNN`, undefined, { field: `${field}[${index}]` });
    return findingId;
  });
...
function normalizeCardDispositions(value) {
  if (!Array.isArray(value) || value.length !== CARD_IDENTITIES.length) fail("cardDispositions must contain exactly thirteen cards", undefined, { field: "cardDispositions" });
  return value.map((card, index) => {
    const field = `cardDispositions[${index}]`;
    assertExactKeys(card, ["batchId", "decisionId", "questionId", "disposition", "requiredReviewDomain", "evidenceRefs", "findingIds"], field);
    const identity = {
      batchId: normalizeEnum(card.batchId, ["BATCH_1", "BATCH_2", "BATCH_3"], `${field}.batchId`),
...
function normalizeInvariantResults(value) {
  if (!Array.isArray(value) || value.length !== INVARIANT_IDS.length) fail("crossBatchInvariantResults must contain exactly twelve results", undefined, { field: "crossBatchInvariantResults" });
  return value.map((invariant, index) => {
    const field = `crossBatchInvariantResults[${index}]`;
...
function normalizeAttestations(value, expectedContentSha256) {
  ...
  const normalized = value.map((attestation, index) => {
    ...
    if (!/^D040-FTB-IR-ATTEST-R\d{3}$/.test(attestationId)) fail(`${field}.attestationId must be D040-FTB-IR-ATTEST-RNNN`, undefined, { field: `${field}.attestationId` });
    ...
    if (supersedesAttestationId !== null && (!/^D040-FTB-IR-ATTEST-R\d{3}$/.test(supersedesAttestationId) || supersedesAttestationId === attestationId)) {
...
function reviewContentProjection(input) {
  return {
    ...
    crossBatchInvariantResults: input.crossBatchInvariantResults,
...
function computeD040FirstThreeBatchesReviewContentSha256(input) {
  assertDataTree(input, "input");
  return fingerprint(reviewContentProjection(input));
}

function computeD040FirstThreeBatchesReviewBundleSha256(input) {
  assertDataTree(input, "input");
...
function validateFindingLinks(cards, invariants, findings) {
  ...
  for (const invariant of invariants) {
    for (const findingId of invariant.findingIds) {
      if (!findingById.has(findingId)) fail("invariant references an unknown finding", undefined, { field: "crossBatchInvariantResults.findingIds" });
      referencedIds.add(findingId);
    }
  }
  ...
  for (const invariant of invariants.filter(({ result }) => result === "PASS")) {
    ...
    })) fail("a passing invariant cannot cite an open blocking finding", undefined, { field: "crossBatchInvariantResults" });
  }
}
...
function normalizeD040FirstThreeBatchesIndependentReviewRecord(input) {
  assertDataTree(input, "input");
  assertExactKeys(input, [
    "schemaVersion", "recordKind", "reviewId", "packetIdentity", "reviewedArtifacts", "cardDispositions",
    "crossBatchInvariantResults", "findings", "overallDisposition", "reviewedAt", "supersedesReviewId",
  ...
  const expectedReviewPattern = recordKind === "FORMAL_REVIEW_RECORD"
    ? /^D040-FTB-REVIEW-R\d{3}$/
    : /^D040-FTB-SYNTHETIC-R\d{3}$/;
  ...
  const crossBatchInvariantResults = normalizeInvariantResults(input.crossBatchInvariantResults);
  ...
  if (containsCredential || containsIdentityDocument || containsSignatureMaterial) {
    fail("review bundles cannot contain credentials, identity documents, or signature material", "UNSAFE_D040_FIRST_THREE_BATCHES_INDEPENDENT_REVIEW_RECORD", { field: "containsProhibitedMaterial" });
  }
  const contentCore = {
  ...
  validateFindingLinks(cardDispositions, crossBatchInvariantResults, findings);
  const derivedDisposition = deriveOverallDisposition(cardDispositions, crossBatchInvariantResults, findings, reviewerAttestations);
...
function evaluateD040FirstThreeBatchesIndependentReviewRecord(input) {
  ...
  const invariantResultCounts = Object.fromEntries(INVARIANT_RESULT_IDS.map((id) => [id, normalized.crossBatchInvariantResults.filter(({ result }) => result === id).length]));
  ...
  const wouldBeFirstThreeBatchesIndependentReviewPassCandidate = normalized.overallDisposition === "INDEPENDENT_REVIEW_PASS_CANDIDATE";
  const firstThreeBatchesIndependentReviewPassCandidate = normalized.recordKind === "FORMAL_REVIEW_RECORD" && wouldBeFirstThreeBatchesIndependentReviewPassCandidate;
  const blockers = immutable([
    ...
    "DYNAMIC_MODEL_OPTIONS_NOT_OWNER_READY",
  ...
  const core = immutable({
    ...
    disposition: normalized.recordKind === "SYNTHETIC_CONTRACT_FIXTURE"
      ...
      : firstThreeBatchesIndependentReviewPassCandidate
    ...
    wouldBeFirstThreeBatchesIndependentReviewPassCandidate,
    firstThreeBatchesIndependentReviewPassCandidate,
    firstThreeBatchesIndependentReviewPassed: false,
...
function validateD040FirstThreeBatchesIndependentReviewRecordResult(result, input) {
  assertDataTree(result, "result");
  const expected = evaluateD040FirstThreeBatchesIndependentReviewRecord(input);
  if (!isDeepStrictEqual(result, expected)) fail("D-040 first-three-batches independent review result or fingerprint was changed");
  return expected;
}
