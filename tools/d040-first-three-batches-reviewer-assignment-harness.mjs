...
const CONTRACT_ID = "D040-FIRST-THREE-BATCHES-REVIEWER-ASSIGNMENT-CONTRACT-001";
const INPUT_SCHEMA_VERSION = "D040_FIRST_THREE_BATCHES_REVIEWER_ASSIGNMENT_INPUT_V1";
const RESULT_SCHEMA_VERSION = "D040_FIRST_THREE_BATCHES_REVIEWER_ASSIGNMENT_RESULT_V1";
const BOUNDARY_SCHEMA_VERSION = "D040_FIRST_THREE_BATCHES_REVIEWER_ASSIGNMENT_BOUNDARY_V1";
const INTAKE_PACKET_ID = "D040-FIRST-THREE-BATCHES-REVIEWER-INTAKE-PACKET-001";
...
function fail(message, code = "INVALID_D040_FIRST_THREE_BATCHES_REVIEWER_ASSIGNMENT", details = {}) {
...
function assertSafeFieldName(key, field) {
  ...
  if (SENSITIVE_FIELD_NAMES.has(normalized)) {
    fail("input contains a prohibited sensitive field", "UNSAFE_D040_FIRST_THREE_BATCHES_REVIEWER_ASSIGNMENT", { field });
  }
}

function assertSafeString(value, field) {
  if (containsSensitiveLookingValue(value)) {
    fail("input contains prohibited sensitive-looking material", "UNSAFE_D040_FIRST_THREE_BATCHES_REVIEWER_ASSIGNMENT", { field });
  }
}
...
function assertPersonName(value, field, recordKind) {
  ...
  const roleOnly = new Set([
    ...
    "product decision quality", "privacy data integrity", "health formula evidence",
    "qa accessibility",
  ]);
...
function normalizeCandidate(value, index, recordKind) {
  ...
  const candidatePattern = recordKind === SYNTHETIC_RECORD_KIND
    ? /^D040-FIRST-THREE-BATCHES-SYNTHETIC-C\d{3}$/
    : /^D040-FIRST-THREE-BATCHES-REVIEWER-C\d{3}$/;
  ...
  if (
    !Array.isArray(value.proposedReviewDomains) ||
    value.proposedReviewDomains.length === 0 ||
    value.proposedReviewDomains.length > REVIEW_DOMAINS.length
  ) {
    fail(`${field}.proposedReviewDomains must contain one to four domains`, undefined, { field: `${field}.proposedReviewDomains` });
  }
...
function normalizeD040FirstThreeBatchesReviewerAssignment(input) {
  ...
  const assignmentPattern = input.recordKind === SYNTHETIC_RECORD_KIND
    ? /^D040-FIRST-THREE-BATCHES-SYNTHETIC-A\d{3}$/
    : /^D040-FIRST-THREE-BATCHES-ASSIGNMENT-A\d{3}$/;
  ...
  if (!Array.isArray(input.domainCoverage) || input.domainCoverage.length !== REVIEW_DOMAINS.length) {
    fail("domainCoverage must contain exactly four domains", undefined, { field: "domainCoverage" });
  }
  ...
  for (const flag of ["containsCredential", "containsIdentityDocument", "containsPrivateContact", "containsSignatureMaterial"]) {
    if (input[flag] !== false) fail(`${flag} must be false`, "UNSAFE_D040_FIRST_THREE_BATCHES_REVIEWER_ASSIGNMENT", { field: flag });
  }
...
function evaluateD040FirstThreeBatchesReviewerAssignment(input) {
...
function validateD040FirstThreeBatchesReviewerAssignmentResult(result, input) {
  assertDataTree(result, "result");
  const expected = evaluateD040FirstThreeBatchesReviewerAssignment(input);
