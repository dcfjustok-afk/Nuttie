...
const CONTRACT_ID = "D040-NDB-REVIEWER-ASSIGNMENT-CONTRACT-001";
const INPUT_SCHEMA_VERSION = "D040_NON_DIAGNOSTIC_BOUNDARY_REVIEWER_ASSIGNMENT_INPUT_V1";
const RESULT_SCHEMA_VERSION = "D040_NON_DIAGNOSTIC_BOUNDARY_REVIEWER_ASSIGNMENT_RESULT_V1";
const BOUNDARY_SCHEMA_VERSION = "D040_NON_DIAGNOSTIC_BOUNDARY_REVIEWER_ASSIGNMENT_BOUNDARY_V1";
const INTAKE_PACKET_ID = "D040-NON-DIAGNOSTIC-BOUNDARY-REVIEWER-INTAKE-PACKET-001";
...
function fail(message, code = "INVALID_D040_NDB_REVIEWER_ASSIGNMENT", details = {}) {
...
function assertSafeFieldName(key, field) {
  ...
  if (SENSITIVE_FIELD_NAMES.has(normalized)) {
    fail("input contains a prohibited sensitive field", "UNSAFE_D040_NDB_REVIEWER_ASSIGNMENT", { field });
  }
}

function assertSafeString(value, field) {
  if (containsSensitiveLookingValue(value)) {
    fail("input contains prohibited sensitive-looking material", "UNSAFE_D040_NDB_REVIEWER_ASSIGNMENT", { field });
  }
}
...
function normalizeCandidate(value, index, recordKind) {
  ...
  const candidatePattern = recordKind === SYNTHETIC_RECORD_KIND
    ? /^D040-NDB-SYNTHETIC-C\d{3}$/
    : /^D040-NDB-REVIEWER-C\d{3}$/;
...
function normalizeD040NonDiagnosticBoundaryReviewerAssignment(input) {
  ...
  const assignmentPattern = input.recordKind === SYNTHETIC_RECORD_KIND
    ? /^D040-NDB-SYNTHETIC-A\d{3}$/
    : /^D040-NDB-ASSIGNMENT-A\d{3}$/;
  ...
  for (const flag of ["containsCredential", "containsIdentityDocument", "containsPrivateContact", "containsSignatureMaterial"]) {
    if (input[flag] !== false) fail(`${flag} must be false`, "UNSAFE_D040_NDB_REVIEWER_ASSIGNMENT", { field: flag });
  }
...
function evaluateD040NonDiagnosticBoundaryReviewerAssignment(input) {
...
function validateD040NonDiagnosticBoundaryReviewerAssignmentResult(result, input) {
  assertDataTree(result, "result");
  const expected = evaluateD040NonDiagnosticBoundaryReviewerAssignment(input);
