...
} from "./d040-macro-axis-independent-review-record-harness.mjs";
...
function attestation(index, reviewDomain) {
  ...
  return {
    attestationId: `D040-MA-IR-ATTEST-R${number}`,
...
function finalize(input) {
  input.reviewContentSha256 = computeD040MacroAxisReviewContentSha256(input);
  for (const reviewer of input.reviewerAttestations) reviewer.reviewContentSha256 = input.reviewContentSha256;
  input.bundleSha256 = computeD040MacroAxisReviewBundleSha256(input);
  return input;
}

function finding({
  findingId = "D040-MA-IR-F001",
  severity = "P3",
...
test("locks the V1 review bundle, frozen packet, four domains, four cards, and fourteen invariants", () => {
  assert.equal(INPUT_SCHEMA_VERSION, "D040_MACRO_AXIS_INDEPENDENT_REVIEW_BUNDLE_INPUT_V1");
  assert.equal(RESULT_SCHEMA_VERSION, "D040_MACRO_AXIS_INDEPENDENT_REVIEW_RESULT_V1");
  assert.equal(CONTRACT_ID, "D040-MACRO-AXIS-INDEPENDENT-REVIEW-RECORD-CONTRACT-001");
  assert.equal(PACKET_IDENTITY.packetVersion, "PACKET-001-R1");
  assert.equal(PACKET_IDENTITY.packetEventId, "EVT-20260821-006");
  assert.equal(PACKET_IDENTITY.inputManifestEventId, "EVT-20260821-007");
  assert.equal(PACKET_IDENTITY.inputCommit, "47ba4895dac2535682e8d1a8cb985176d6ad45f7");
  assert.equal(PACKET_IDENTITY.manifestRecordCommit, "d8e812f1324590d735f809ea994e8aaa2f6805d8");
  assert.equal(REVIEWED_ARTIFACTS.length, 10);
  assert.equal(REVIEW_DOMAINS.length, 4);
  assert.equal(CARD_IDENTITIES.length, 4);
  assert.equal(INVARIANT_IDS.length, 14);
...
test("accepts a complete synthetic fixture only as non-evidence structural coverage", () => {
  const result = evaluateD040MacroAxisIndependentReviewRecord(finalize(baseInput()));
  ...
  assert.equal(result.wouldBeMacroAxisIndependentReviewPassCandidate, true);
  assert.equal(result.macroAxisIndependentReviewPassCandidate, false);
  assert.equal(result.macroAxisIndependentReviewPassed, false);
  ...
  assert.equal(
    result.blockers.includes("D063_D070_NOT_ACCEPTED_AND_MACRO_AXIS_CARDS_NOT_OWNER_READY"),
    true,
  );
...
test("a structurally complete formal record is still only a caller-asserted pass candidate", () => {
  const result = evaluateD040MacroAxisIndependentReviewRecord(finalize(baseInput("FORMAL_REVIEW_RECORD")));
  assert.equal(result.disposition, "STRUCTURALLY_COMPLETE_REVIEW_ONLY");
  assert.equal(result.macroAxisIndependentReviewPassCandidate, true);
  assert.equal(result.macroAxisIndependentReviewPassed, false);
  ...
  assert.equal(result.boundary.d072OwnerReady, false);
...
test("rejects packet identity, packet artifact, or any frozen reviewed artifact drift", () => {
  ...
  for (const mutate of mutations) {
    ...
    mutate(input);
    assert.throws(() => evaluateD040MacroAxisIndependentReviewRecord(input), { code: "INVALID_D040_MACRO_AXIS_INDEPENDENT_REVIEW_RECORD" });
  }
});
...
test("uncountable drafting, identity, conflict, or signature attestations derive INCOMPLETE", () => {
  ...
  for (const mutate of mutations) {
    ...
    const result = evaluateD040MacroAxisIndependentReviewRecord(finalize(input));
...
test("REJECT_SPEC, failed invariants, or open P0 derive REJECTED with highest priority", () => {
  ...
  rejectedCard.cardDispositions[0].findingIds = ["D040-MA-IR-F001"];
  rejectedCard.findings = [finding({ severity: "P0" })];
  rejectedCard.overallDisposition = "REJECTED";
  assert.equal(evaluateD040MacroAxisIndependentReviewRecord(finalize(rejectedCard)).overallDisposition, "REJECTED");

  const failedInvariant = baseInput();
  failedInvariant.crossAxisInvariantResults[0].result = "FAIL";
  failedInvariant.crossAxisInvariantResults[0].findingIds = ["D040-MA-IR-F001"];
  failedInvariant.findings = [finding()];
  failedInvariant.overallDisposition = "REJECTED";
  assert.equal(evaluateD040MacroAxisIndependentReviewRecord(finalize(failedInvariant)).overallDisposition, "REJECTED");

  const forged = clone(rejectedCard);
  forged.overallDisposition = "CHANGES_REQUIRED";
  finalize(forged);
  assert.throws(() => evaluateD040MacroAxisIndependentReviewRecord(forged), { code: "INVALID_D040_MACRO_AXIS_INDEPENDENT_REVIEW_RECORD" });
});
...
test("required card changes or open P1/P2 findings derive CHANGES_REQUIRED", () => {
  for (const severity of ["P1", "P2"]) {
    ...
    input.cardDispositions[0].findingIds = ["D040-MA-IR-F001"];
...
test("OUT_OF_SCOPE, NOT_VERIFIED, or missing domain coverage derive INCOMPLETE", () => {
  ...
  outOfScope.cardDispositions[0].findingIds = ["D040-MA-IR-F001"];
  outOfScope.findings = [finding()];
  outOfScope.overallDisposition = "INCOMPLETE";
  assert.equal(evaluateD040MacroAxisIndependentReviewRecord(finalize(outOfScope)).overallDisposition, "INCOMPLETE");

  const notVerified = baseInput();
  notVerified.crossAxisInvariantResults[13].result = "NOT_VERIFIED";
  notVerified.crossAxisInvariantResults[13].evidenceRefs = [];
  notVerified.overallDisposition = "INCOMPLETE";
  assert.equal(evaluateD040MacroAxisIndependentReviewRecord(finalize(notVerified)).invariantResultCounts.NOT_VERIFIED, 1);
  ...
  assert.equal(evaluateD040MacroAxisIndependentReviewRecord(finalize(missingDomain)).countableAttestationCount, 3);
});

test("requires exact four-card and fourteen-invariant identity, order, and coverage", () => {
  const mutations = [
    ...
    (input) => { input.crossAxisInvariantResults[0].invariantId = "D040-MA-XAI-014"; },
...
test("enforces bidirectional finding references and rejects unknown, orphan, or unsafe PASS links", () => {
  ...
  unknown.cardDispositions[0].findingIds = ["D040-MA-IR-F999"];
  assert.throws(() => evaluateD040MacroAxisIndependentReviewRecord(finalize(unknown)), { code: "INVALID_D040_MACRO_AXIS_INDEPENDENT_REVIEW_RECORD" });

  const orphan = baseInput();
  orphan.findings = [finding()];
  assert.throws(() => evaluateD040MacroAxisIndependentReviewRecord(finalize(orphan)), { code: "INVALID_D040_MACRO_AXIS_INDEPENDENT_REVIEW_RECORD" });

  const mismatch = baseInput();
  mismatch.cardDispositions[0].findingIds = ["D040-MA-IR-F001"];
  mismatch.findings = [finding({ decisionIds: ["D-072"] })];
  assert.throws(() => evaluateD040MacroAxisIndependentReviewRecord(finalize(mismatch)), { code: "INVALID_D040_MACRO_AXIS_INDEPENDENT_REVIEW_RECORD" });

  const unsafePass = baseInput();
  unsafePass.crossAxisInvariantResults[0].findingIds = ["D040-MA-IR-F001"];
  unsafePass.findings = [finding({ severity: "P2" })];
  unsafePass.overallDisposition = "CHANGES_REQUIRED";
  assert.throws(() => evaluateD040MacroAxisIndependentReviewRecord(finalize(unsafePass)), { code: "INVALID_D040_MACRO_AXIS_INDEPENDENT_REVIEW_RECORD" });
});
...
test("allows only fully owned future-dated open P3 findings as non-blocking review material", () => {
  ...
  for (const mutate of [
    ...
    invalid.cardDispositions[0].findingIds = ["D040-MA-IR-F001"];
    invalid.findings = [finding()];
    mutate(invalid.findings[0]);
    assert.throws(() => evaluateD040MacroAxisIndependentReviewRecord(finalize(invalid)), { code: "INVALID_D040_MACRO_AXIS_INDEPENDENT_REVIEW_RECORD" });
  }
});

test("closed findings require closure evidence and cannot retain open-P3 disposition fields", () => {
  const closed = baseInput();
  closed.cardDispositions[0].findingIds = ["D040-MA-IR-F001"];
  closed.findings = [finding({ state: "CLOSED", severity: "P2" })];
  assert.equal(evaluateD040MacroAxisIndependentReviewRecord(finalize(closed)).overallDisposition, "INDEPENDENT_REVIEW_PASS_CANDIDATE");

  const missingClosure = clone(closed);
  missingClosure.findings[0].closureEvidenceRefs = [];
  assert.throws(() => evaluateD040MacroAxisIndependentReviewRecord(finalize(missingClosure)), { code: "INVALID_D040_MACRO_AXIS_INDEPENDENT_REVIEW_RECORD" });

  const staleP3Fields = baseInput();
  staleP3Fields.cardDispositions[0].findingIds = ["D040-MA-IR-F001"];
  staleP3Fields.findings = [finding({ state: "CLOSED" })];
  staleP3Fields.findings[0].accountableOwnerRef = "STALE-OWNER-REF";
  assert.throws(() => evaluateD040MacroAxisIndependentReviewRecord(finalize(staleP3Fields)), { code: "INVALID_D040_MACRO_AXIS_INDEPENDENT_REVIEW_RECORD" });
});
...
test("binds review content, every attestation, and the complete bundle with two SHA-256 layers", () => {
  ...
  assert.throws(() => evaluateD040MacroAxisIndependentReviewRecord(contentDrift), { code: "INVALID_D040_MACRO_AXIS_INDEPENDENT_REVIEW_RECORD" });
  ...
  attestationContentDrift.bundleSha256 = computeD040MacroAxisReviewBundleSha256(attestationContentDrift);
  assert.throws(() => evaluateD040MacroAxisIndependentReviewRecord(attestationContentDrift), { code: "INVALID_D040_MACRO_AXIS_INDEPENDENT_REVIEW_RECORD" });
  ...
  assert.throws(() => evaluateD040MacroAxisIndependentReviewRecord(signatureDrift), { code: "INVALID_D040_MACRO_AXIS_INDEPENDENT_REVIEW_RECORD" });

  const bundleDrift = finalize(baseInput());
  bundleDrift.bundleSha256 = digest("forged bundle");
  assert.throws(() => evaluateD040MacroAxisIndependentReviewRecord(bundleDrift), { code: "INVALID_D040_MACRO_AXIS_INDEPENDENT_REVIEW_RECORD" });
});
...
test("rejects duplicate attestation, reviewer, signature, or competence identities", () => {
  ...
  for (const mutate of mutations) {
    const input = baseInput();
    mutate(input);
    assert.throws(() => evaluateD040MacroAxisIndependentReviewRecord(finalize(input)), { code: "INVALID_D040_MACRO_AXIS_INDEPENDENT_REVIEW_RECORD" });
  }
});
...
test("separates formal and synthetic review IDs and validates supersession identities", () => {
  ...
  formalWithSyntheticId.reviewId = "D040-MA-SYNTHETIC-R001";
  assert.throws(() => evaluateD040MacroAxisIndependentReviewRecord(finalize(formalWithSyntheticId)), { code: "INVALID_D040_MACRO_AXIS_INDEPENDENT_REVIEW_RECORD" });

  const syntheticWithFormalId = baseInput();
  syntheticWithFormalId.reviewId = "D040-MA-REVIEW-R001";
  assert.throws(() => evaluateD040MacroAxisIndependentReviewRecord(finalize(syntheticWithFormalId)), { code: "INVALID_D040_MACRO_AXIS_INDEPENDENT_REVIEW_RECORD" });

  const selfSupersession = baseInput();
  selfSupersession.supersedesReviewId = selfSupersession.reviewId;
  assert.throws(() => evaluateD040MacroAxisIndependentReviewRecord(finalize(selfSupersession)), { code: "INVALID_D040_MACRO_AXIS_INDEPENDENT_REVIEW_RECORD" });

  const invalidCalendarDate = baseInput();
  invalidCalendarDate.reviewedAt = "2026-02-31T00:00:00+08:00";
  assert.throws(() => evaluateD040MacroAxisIndependentReviewRecord(finalize(invalidCalendarDate)), { code: "INVALID_D040_MACRO_AXIS_INDEPENDENT_REVIEW_RECORD" });
});
...
test("rejects sensitive-looking material without echoing the canary", () => {
  ...
  let error;
  try {
    evaluateD040MacroAxisIndependentReviewRecord(input);
  } catch (caught) {
    error = caught;
  }
  assert.equal(error?.code, "UNSAFE_D040_MACRO_AXIS_INDEPENDENT_REVIEW_RECORD");
...
test("rejects accessors, symbols, special objects, cycles, sparse arrays, extra fields, and resource overflow", () => {
  ...
  sparse.crossAxisInvariantResults = new Array(14);
  inputs.push(sparse);
...
test("rejects forged result fields or fingerprints", () => {
  const input = finalize(baseInput());
  const result = evaluateD040MacroAxisIndependentReviewRecord(input);
  const mutations = [
    (value) => { value.macroAxisIndependentReviewPassed = true; },
  ...
  for (const mutate of mutations) {
    ...
    mutate(forged);
    assert.throws(() => validateD040MacroAxisIndependentReviewRecordResult(forged, input), { code: "INVALID_D040_MACRO_AXIS_INDEPENDENT_REVIEW_RECORD" });
  }
  assert.deepEqual(validateD040MacroAxisIndependentReviewRecordResult(result, input), result);
});

test("source performs no filesystem, network, process, clock, message, Git, or signature side effect", async () => {
  const source = await readFile(new URL("./d040-macro-axis-independent-review-record-harness.mjs", import.meta.url), "utf8");
  const forbidden = [
    /node:fs/,
    /node:http/,
    /node:https/,
  ...
  assert.equal(result.boundary.macroAxisIndependentReviewPassed, false);
