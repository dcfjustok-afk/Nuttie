# D-039 B03~B05 六卡独立复核人接入与指派检查包

| 字段 | 内容 |
| --- | --- |
| 工件 ID | `D039-B03-B05-REVIEWER-INTAKE-PACKET-001` |
...
| 对应回执合同 | `D039-B03-B05-INDEPENDENT-REVIEW-RECORD-CONTRACT-001` |
| 对应阻断项 | `D039-PX5-B03 / B04 / B05` |
| 当前下一步 | `NAMED_REVIEWER_CANDIDATES_AND_CONTACT_AUTHORIZATION_REQUIRED` |
| 非目标 | 预填人名、核验现实身份或胜任、发送消息、开始复核、关闭 B03~B05、替 Owner 选择或授权实现 |

## 1. 用途与当前真实状态

[D-039 B03~B05 六卡独立复核包](d039-b03-b05-independent-review-packet.md)已经冻结十份输入、D-045/D-031/D-033/D-034/D-036/D-053 六卡、四个复核域、十六条跨卡不变量与 P0~P3 标准；[回执机器合同](../04-engineering/testing/d039-independent-review-record-contract.md)及[本地 validator](../04-engineering/testing/d039-independent-review-record-harness.md)只校验未来调用方提供的回执数据。本检查包补齐两者之间的现实接入步骤，让 PM 在取得真实候选人和联络授权后，可以用同一套字段完成候选收集、核验、覆盖检查与正式交接。

当前没有候选人输入，也没有外部联络授权：

- 四个复核域的具名人员仍为 `0`，身份、逐域胜任、独立性、利益冲突和签署方式均未核验；
- 本文件的字段和空白表格不是 assignment record，示例值不是现实证据；
- PM、六卡作者、Owner、Codex/AI、Agent ID 或只有岗位名称不能形成独立复核签署；
- 本检查包不读取联系人、不发送消息，也不保存身份证件、证书原件、私人联系方式、签名图片或访问凭据；
- `reviewersAssigned`、`independentReviewStarted`、`independentReviewPassed`、B03/B04/B05、PX-5、Owner 与全部实现授权继续为 `false`。
...
5. 记录候选人是否参与冻结包或十份受审输入的起草，并披露利益冲突；参与起草或冲突未解决者不能计入任何域覆盖。
6. 合并全部有效候选人后，四个复核域都至少由一名合格人员覆盖；允许重复覆盖，不允许缺域或单向矩阵。
7. PM 生成正式 assignment record，绑定本检查包、冻结复核包、候选人接受记录、核验引用、联络授权和域覆盖矩阵。

“指派完成”只允许启动正式复核，不证明任何人已经读完材料、生成 attestation、记录 finding 或形成独立复核 PASS，也不关闭 benchmark、Provider、原生、Owner 或 PX-5 门禁。

## 3. 四域覆盖与逐域胜任要求

| reviewDomain | 可接受的胜任依据方向 | 必须排除的替代物 | 具名候选人 | 当前状态 |
| --- | --- | --- | --- | --- |
| `PRODUCT_DECISION_QUALITY` | 产品决定拆分、选项互斥、依赖排序、验收与 Owner 门禁的可核验经历或交付引用 | 推荐就是答案、作者自审、只看原型、通用产品岗位名称 |  | `UNASSIGNED` |
| `PRIVACY_DATA_INTEGRITY` | 本地优先、数据最小化、保留/删除/备份、事务与失败恢复的可核验经历或交付引用 | 取得或预览即同意保留、清辅助状态即删除事实、框架无关测试绿色 |  | `UNASSIGNED` |
| `SECURITY_TRANSPORT_RESOURCE_EVIDENCE` | 移动端安全、URL/redirect/session、Provider 用途、资源预算、benchmark 与证据强度的可核验经历或交付引用 | Windows Metro export、合成 fixture、Provider 自述或未执行协议 |  | `UNASSIGNED` |
| `QA_ACCESSIBILITY` | 需求—验收—证据追踪、负向路径、Dynamic Type、VoiceOver、小屏和门禁测试的可核验经历或交付引用 | 测试计划、作者自测、局部单测或只有 QA 角色名 |  | `UNASSIGNED` |
...
```text
主题：Nuttie D-039 B03~B05 六卡独立复核接入确认

我们希望邀请你以具名复核人的身份审查
D039-B03-B05-INDEPENDENT-REVIEW-PACKET-001 / PACKET-001-R1。

拟覆盖复核域：<逐项列出>
请先确认：
1) 是否接受该 packet revision、复核域和预计完成时间；
2) 每个复核域的胜任依据；
3) 是否参与十份冻结输入的起草及是否存在利益冲突；
4) 可使用的受控签署方式。

项目只在仓库记录必要核验结论和非敏感引用，不收集证件原件、
私人联系方式或签名材料。完成接入只表示可以开始复核，
不关闭 B03~B05，不替代 benchmark、Provider/原生证据，
也不授权 Owner 评审、AI 发送、媒体保留、正式根工程或实现。
```
...
3. 复核人按六卡、十六条跨卡不变量和 finding 规则执行独立审查。
4. 每位复核人生成绑定同一 `reviewContentSha256` 的 attestation。
5. 使用[本地回执 validator](../04-engineering/testing/d039-independent-review-record-harness.md)做结构检查；candidate 结果仍须由获授权人员核验现实身份、逐域胜任、独立性和签署工件。
6. 只有正式复核记录满足合同且现实核验完成，才能另行登记独立复核结论；B03/B04/B05 仍需对应 Owner 选择和各自外部/原生证据关闭。
