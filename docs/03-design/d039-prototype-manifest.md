...
本原型在 PX-2 阶段只用于比较 D-039 的“添加餐食首层入口如何组织”。Owner 已于 2026-08-15 查看冻结原型后明确回复 `a`，方案 A 已写入权威决定台账；原型页面本身仍不保存或改变决定：

| 方案 | 唯一变化轴 | 固定能力 | 当前状态 |
| --- | --- | --- | --- |
| A | 本地搜索和最近使用先出现；扫描与 AI 并列 | 搜索、最近、条码、自建、AI 状态、统一编辑与本地保存全部可达 | `ACCEPTED / PX-4_BASELINE` |
| B | 记住上次方式；可模拟首次、上次本地搜索、上次 AI 图片三种状态 | 同 A；记住 AI 入口不记住发送同意，图片仍逐次预览 | `NOT_SELECTED / REFERENCE_ONLY` |
| C | 搜索、扫描、拍照、相册、文字、自建食品六种方式在首层平铺 | 同 A；最近使用仍可到达 | `NOT_SELECTED / REFERENCE_ONLY` |

页面初始显示 A 的历史原因是字母顺序，而不是页面默认值替 Owner 做出选择。切换 A/B/C 和评审状态仍只修改当前页面内存；刷新后复位，不写入决定台账。当前权威选择来自 Owner 明确回复和 `EVT-20260815-001`，正式规格见 [D-039 PX-4 设计基线](d039-px4-design-baseline.md)。
...
独立复测结论为 `PASS`：仓库与实验室三对文件逐字节一致，两份 QA 均为 19/19 flows、外部请求 0、运行时问题 0；额外逐一点击相机/相册拒绝态的 10 个恢复出口，落点、标题焦点与零写入均正确。D039-QA-001 至 D039-QA-010 全部关闭，PX-2 升级为 `PASS`。这份 PX-2 结论本身不代表 Owner 选择；后续 Owner 的明确 A 选择由独立的 `EVT-20260815-001` 记录。两者均不授权正式 React Native 实现。
...
- 原型不保存 Owner 点击、偏好、评价或可用性结果；D-039 当前正式决定来自 Owner 明确文字回复和权威台账，不来自原型默认状态。
...
3. PX-5 DoR 已完成首次评估，结论为 `NOT_READY`；B01 验收矩阵与 B02 路由/可观测性规格已关闭，B03~B07 仍开放。正式页面、真实相机/相册、AI transport、数据库、原生与发布工作继续未授权；精确阻断项见 [PX-5 实现就绪评估](../05-quality/d039-px5-dor-assessment.md)。
