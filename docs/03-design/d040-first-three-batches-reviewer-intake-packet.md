# D-040 前三批十三卡独立复核人接入与指派检查包

| 字段 | 内容 |
| --- | --- |
| 工件 ID | `D040-FIRST-THREE-BATCHES-REVIEWER-INTAKE-PACKET-001` |
...
| 对应回执合同 | `D040-FIRST-THREE-BATCHES-INDEPENDENT-REVIEW-RECORD-CONTRACT-001` |
...
[D-040 前三批十三卡独立复核包](d040-first-three-batches-independent-review-packet.md)已经冻结七份输入、四个复核域、十三卡、十二条跨批不变量与 P0~P3 标准；[回执机器合同](../04-engineering/testing/d040-first-three-batches-independent-review-record-contract.md)及[本地 validator](../04-engineering/testing/d040-first-three-batches-independent-review-record-harness.md)只检查未来调用方提供的回执数据。本检查包补齐两者之间的现实接入步骤，让 PM 在取得真实候选人和联络授权后，按同一冻结 packet 完成候选收集、核验、覆盖检查与正式交接。

当前没有候选人输入，也没有外部联络授权：

- 四个复核域的具名人员仍为 `0`，身份、逐域胜任、独立性、利益冲突和签署方式均未核验；
- 本文件的字段与空白表格不是 assignment record，示例值不是现实证据；
- PM、十三卡作者、Owner、Codex/AI、Agent ID 或只有岗位名称不能形成独立复核签署；
- 健康/公式域卡片复核不能代替 `ChinaQualifiedHealthReviewer` 对中国健康文案、支持资源和人群边界的正式签署；
- 本检查包不读取联系人、不发送消息，也不保存身份证件、证书原件、私人联系方式、签名图片或访问凭据；
- `reviewersAssigned`、`independentReviewStarted`、`firstThreeBatchesIndependentReviewPassed`、PX-1/PX-2、Owner 与全部实现授权继续为 `false`。
...
5. 记录候选人是否参与冻结包、七份受审输入或十三卡的起草，并披露利益冲突；参与起草或冲突未解决者不能计入任何域覆盖。
...
```text
主题：Nuttie D-040 前三批十三卡独立复核接入确认
...
3. 复核人按十三卡、十二条跨批不变量和 finding 规则执行独立审查。
4. 每位复核人生成绑定同一 `reviewContentSha256` 的 attestation。
5. 使用[本地回执 validator](../04-engineering/testing/d040-first-three-batches-independent-review-record-harness.md)做结构检查；candidate 结果仍须由获授权人员核验现实身份、逐域胜任、独立性和签署工件。
6. 只有正式复核记录满足合同且现实核验完成，才能另行登记独立复核结论；动态模型、健康批准、Content QA、宏量轴、Owner 与实现仍按各自门禁关闭。
