...
| 版本 D-032 | Expo SDK 57 / RN 0.86.2 隔离 Spike | `CANDIDATE + SPIKE_AUTHORIZED`；Windows JS 依赖表面、Android 与 iOS 平台条件 export 子范围 PASS；Android/iOS export 结构已共用核心自动校验，字节数/SHA 不作可复现构建门禁 | Prebuild、SQLCipher、SecureStore、相机、通知、Xcode/CocoaPods、原生编译/运行、Archive、真机后再由 Owner 冻结 |
