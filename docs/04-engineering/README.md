...
- [D-034 benchmark raw run 与报告 bundle 机器合同](testing/d034-benchmark-run-report-contract.md)：固定 raw run 身份/阶段/指标、最低 2,550 measured + 765 warm-up、整组丢弃/重试保留、nearest-rank p95、聚合重算和 measured candidate 不等于 benchmark pass；当前无 run、无报告、无执行授权。
- [D-034 benchmark raw run/report 本地校验合同](testing/d034-benchmark-run-report-harness.md)：以 17 项测试执行严格身份/阶段/指标、最低采样公式、三轮换整组丢弃、重试保留、聚合重算、disposition、脱敏和零授权边界；合成记录只验证算法，不登记为 benchmark run/report。
...
- [D-040 前三批十三卡独立复核人指派本地校验合同](testing/d040-first-three-batches-reviewer-assignment-harness.md)：以 21 项测试执行 frozen packet、候选身份/逐域胜任/起草/冲突、四域覆盖、联络/时序/签署、SHA-256、敏感信息与正式/合成隔离；不读取联系人或创建正式指派。
...
- [D-040 D-068/D-069 非诊断边界正式指派记录空白模板](testing/d040-non-diagnostic-boundary-formal-assignment-record-template.md)：固定未来 formal assignment record 的 10 个必备 section、8 个绑定项、敏感存储边界和 start-gate vector；当前仍为 `EMPTY_RECORD_ONLY`，不创建候选人、联系人、正式指派、复核、健康批准、Content QA、Owner/PX 或实现授权。
- [D-040 D-068/D-069 非诊断边界联络授权记录合同](testing/d040-non-diagnostic-boundary-contact-authorization-record-contract.md)：固定未来非敏感 authorization record 的 12 个字段、prior event 绑定、敏感存储禁区和过期/撤销规则；当前 `authorizationRecordCount=0`，不推断联系人授权、材料发送、正式指派、复核、Owner/PX 或实现。
- [D-040 D-068/D-069 非诊断边界候选名单记录合同](testing/d040-non-diagnostic-boundary-reviewer-candidate-roster-contract.md)：固定未来非敏感 reviewer candidate roster 的 14 个字段、11 个 prior event 绑定、9 类禁止存储敏感信息和 9 条验收/过期规则；当前 `reviewerCandidateRosterCount=0`，不推断真实候选人、联系人授权、材料发送、正式指派、复核、Owner/PX 或实现。
...
- [D-040 D-068/D-069 非诊断边界卡规格](../03-design/d040-non-diagnostic-boundary-card-spec.md)：固定慢病/用药上下文暂停、饮食失调风险硬停、plain-language 不确定性和 validated numeric uncertainty 的本地边界；推荐项不等于 Owner 选择。
- [D-040 D-068/D-069 非诊断边界卡本地校验合同](testing/d040-non-diagnostic-boundary-card-harness.md)：以 17 项测试执行 D-068/D-069 六个候选项、YES/UNSURE fail-closed、健康文本/持久化/外联/自动拨号/HealthKit 禁止、合成隔离与 Owner/PX/实现全关闭边界；不诊断、不治疗、不读取或写入健康数据。
- [D-040 D-068/D-069 非诊断边界独立复核回执机器合同](testing/d040-non-diagnostic-boundary-independent-review-record-contract.md)：固定 `PACKET-001-R1` 的 8 份不可变输入、四域具名 attestation、D-068/D-069 两卡、10 条跨卡不变量、P0~P3、双层摘要和 PASS candidate 不等于真实复核通过；当前无 reviewer、健康批准、Content QA 或正式回执。
- [D-040 D-068/D-069 非诊断边界独立复核回执本地校验合同](testing/d040-non-diagnostic-boundary-independent-review-record-harness.md)：以 20 项测试执行 packet/8 输入、四域 attestation、两卡/10 不变量、finding、disposition、双层摘要、合成隔离与零授权边界；不核验现实身份或创建正式回执。
- [D-040 D-068/D-069 非诊断边界独立复核人指派机器合同](testing/d040-non-diagnostic-boundary-reviewer-assignment-contract.md)：固定同一 frozen packet、四域逐域胜任、身份/起草/冲突/联络授权、覆盖矩阵、时间、签署计划与 assignment SHA-256；完整正式输入只产生结构 candidate，当前无联系人、指派、复核、健康批准或 Content QA。
- [D-040 D-068/D-069 非诊断边界独立复核人指派本地校验合同](testing/d040-non-diagnostic-boundary-reviewer-assignment-harness.md)：以 21 项测试执行 frozen packet、候选身份/逐域胜任/起草/冲突、四域覆盖、联络授权/时序/签署、SHA-256、敏感信息与正式/合成隔离；不读取联系人、不外联或创建正式指派。
...
- [D-036 Provider/原生兼容报告本地校验合同](testing/d036-provider-native-compatibility-report-harness.md)：以 20 项测试重算 OI-07 双 fingerprint、36-cell/三路径 raw attempt、108 项离线矩阵、39 项原生 surface、逐 origin/状态/生命周期计数、P0~P3、disposition 与多层 SHA-256；只检查调用方数据，不读取 capture/native 工件，不创建 run/report、联网、复核或 PASS。
...
- [D-053 Provider 证据与 App Privacy 报告机器合同](testing/d053-provider-evidence-app-privacy-report-contract.md)：固定同一 OI-07 revision、3 Provider × 5 payload 的 15 个 profile、150 项十维 assessment、45 项 A-B-C comparison、来源/快照失效、App Privacy/隐私政策/D-033 映射、三角色签名引用、P0~P3、disposition 与多层 SHA-256；调用方声明完整不等于现实证据、签署或准入。
- [D-053 Provider 证据与 App Privacy 报告本地校验合同](testing/d053-provider-evidence-app-privacy-report-harness.md)：以 19 项测试重算 OI-07 双 fingerprint、15/150/45 正式矩阵、来源与映射交叉引用、A/B/C 派生、失效监控、敏感材料、disposition 与不可变结果 fingerprint；合成 fixture 只验证算法，真实 OI-07、Provider、来源/快照、映射、签署、独立复核、Owner、准入、B05、联网和实现均为 0/false。
