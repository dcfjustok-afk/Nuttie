# 首个 MVP 增量范围跨角色复核人指派机器合同

| 字段 | 内容 |
| --- | --- |
| 工件 ID | `MVP-INCREMENT-SCOPE-REVIEWER-ASSIGNMENT-CONTRACT-001` |
| 对应接入包 | `MVP-INCREMENT-SCOPE-REVIEWER-INTAKE-PACKET-001` |
...
| 非目标 | 读取联系人、核验现实身份/胜任、发送邀请、创建正式指派、开始复核、产生 PASS、替 Owner 选择或授权实现 |

## 1. 目的与零事实边界

[复核人接入与指派检查包](../../02-product/mvp-increment-scope-reviewer-intake-packet.md)已经固定五域覆盖、候选人最小字段、身份/逐域胜任/独立性/冲突核验、敏感信息最小化和正式回执交接。本合同进一步固定未来 assignment bundle 的严格 JSON、域覆盖推导、失败关闭状态与摘要，防止遗漏核验、缺域、角色名冒充真人或把接入准备冒充正式指派。
...
输入版本为 `MVP_INCREMENT_SCOPE_REVIEWER_ASSIGNMENT_INPUT_V1`。顶层只允许：
...
正式 ID 使用 `MVP-SCOPE-ASSIGNMENT-ANNN`，测试 ID 使用 `MVP-SCOPE-SYNTHETIC-ANNN`。时间必须是带显式时区的 RFC 3339。所有 nullable 字段都必须显式存在，不得用字段缺失表达 `null`。

资源上限固定为：最多 20 名候选人、每人最多 5 个域、每个引用数组最多 32 项、普通字符串最多 512 UTF-16 code unit、总节点 12,000、深度 16、单数组 128 项。cycle、accessor、symbol、非枚举字段、特殊 prototype、`Map`、`Set`、typed array、稀疏数组、非有限数和额外字段全部拒绝。
...
assignment 只绑定冻结包，不复制或替换十一份受审输入。任一身份值漂移都属于另一 packet revision，既有候选人接受和指派记录不能沿用。
...
正式 candidate ID 使用 `MVP-SCOPE-REVIEWER-CNNN`，合成 ID 使用 `MVP-SCOPE-SYNTHETIC-CNNN`。正式 `reviewerName` 必须是非空真实姓名形态；`PM`、`QA`、`Owner`、`Codex`、`AI`、`Agent`、五个域名、`Example/Test/Synthetic` 或只有岗位名称都拒绝。合成 fixture 必须使用明显的 Example 身份和 `.example.test` 风格非现实引用，不能与正式记录混用。

五个精确域为 `PRODUCT_SCOPE`、`DESIGN_EXPERIENCE`、`ARCHITECTURE_DATA`、`SECURITY_PRIVACY`、`QA_TRACEABILITY`。每位候选人的 `proposedReviewDomains` 必须非空、去重并按固定域顺序排列；`competenceEvidenceByDomain` 必须与其一一对应。
...
本地结构完整候选人还必须满足：未参与起草；身份为 `VERIFIED`；声明域的胜任核验全部为 `VERIFIED`；冲突为 `NONE_DECLARED` 或 `RESOLVED`；接受精确 packet；完成时间晚于接受与正式指派时间；签署方式为 `SIGNED_DOCUMENT_REFERENCE / VERIFIED_WORKFLOW_REFERENCE / WET_SIGNATURE_REFERENCE`；候选人接受时间、签署计划和联络授权引用完整。

本合同对 `participatedInDrafting=true` 统一失败关闭为不计入任何域。现实项目若要允许其审查未参与起草的其他部分，必须另由不参与起草的合格人员覆盖受影响域，且不能通过本 validator 自动放宽。

## 5. 五域覆盖矩阵

`domainCoverage` 必须按固定顺序恰好包含五项：
...
全部候选人声明的域都必须出现在覆盖矩阵中，禁止悬空候选域；矩阵也不能静默遗漏合格候选人的声明域。五域必须各至少有一名合格候选人。允许同一名合格候选人覆盖多域和多人重复覆盖同一域。
...
正式 bundle 只有全部候选记录有效、五域全覆盖、packet 身份精确、联络授权与指派证据完整、声明的 `reviewCanStart=true` 与重算结果一致时，才返回：
...
- 五域固定顺序、逐域胜任证据、双向引用、缺域与悬空候选；
- 起草参与者、PENDING/REJECTED、OPEN 冲突和未授权联络失败关闭；
- packet 接受、指派、截止时间和签署计划的时序；
- 禁止敏感字段名与值且错误不回显；
- `reviewCanStart` 重算、assignment SHA-256、不可变输出、结果重建和源码零副作用审计；
- 合成完整路径不产生 candidate，正式完整路径只产生结构 candidate；
- Git/文件/证件/联系人/签署工件读写、网络、Provider、外部消息、业务写入与正式 assignment record 全部为 0；现实复核人、核验、指派、复核、PASS、Owner、范围冻结、G2、正式工程、原生与实现授权全部为 false。

在获得具名候选人和联络授权前，只允许实现与测试本合同；不得创建假 reviewer、假核验、假联系人、假 assignment、假复核或跨角色 PASS。
