...
24 项 catalog 测试覆盖：来源分区与离线策略、opaque snapshot、前导零 GTIN、跨来源候选、空目录与未命中、非法 GTIN、搜索排序与来源过滤、原文/别名、输入顺序稳定性、缺失/trace/估算/零值、V2 餐食快照往返、单位/基准/status 失败关闭、完整 provenance、展示覆盖、同来源歧义、营养字段拒绝、调用方污染和能力面负向检查。共享 V2 快照另有 4 项直接合同测试。
