# D-040 前三批十三卡独立复核人指派本地校验合同

> 状态：`SPIKE / LOCAL_ONLY / NON_PRODUCTION`
>
> 对应：`D-040 / PX-0_INPUT_GAP / D040-FIRST-THREE-BATCHES-REVIEWER-INTAKE-PACKET-001 / REVIEWERS_UNASSIGNED`
>
> 机器合同：[d040-first-three-batches-reviewer-assignment-contract.md](d040-first-three-batches-reviewer-assignment-contract.md)
>
> 实现：[d040-first-three-batches-reviewer-assignment-harness.mjs](../../../tools/d040-first-three-batches-reviewer-assignment-harness.mjs)；测试：[d040-first-three-batches-reviewer-assignment-harness.test.mjs](../../../tools/d040-first-three-batches-reviewer-assignment-harness.test.mjs)

## 目的

本 validator 把复核人接入包物化为严格、无网络的 `D040_FIRST_THREE_BATCHES_REVIEWER_ASSIGNMENT_INPUT_V1`。它验证冻结 packet 身份、具名候选记录形状、四域逐域胜任、身份核验、起草参与、利益冲突、联络授权、覆盖矩阵、接受/指派/截止时序、真实公历日期、签署计划和 assignment SHA-256。
...
`reviewPacketIdentity` 精确绑定 `D040-FIRST-THREE-BATCHES-INDEPENDENT-REVIEW-PACKET-001 / PACKET-001-R1 / EVT-20260821-001`、input commit/packet artifact commit 及冻结 packet 的 blob OID、SHA-256。四域顺序固定为 Product Decision Quality、Health/Formula Evidence、Privacy/Data Integrity、QA/Accessibility；候选声明域、逐域 competence 和 coverage candidate ID 必须双向一致。
...
输入先检查普通数据树、深度、节点、数组、字符串、cycle、accessor、symbol、非枚举字段、特殊 prototype、稀疏数组与非有限数，再检查精确字段。明显的 key/token/Bearer/Authorization、邮箱、电话、证件号、签名图片、data URL、私钥/证书正文和敏感字段名会触发 `UNSAFE_D040_FIRST_THREE_BATCHES_REVIEWER_ASSIGNMENT`；错误只带稳定字段路径，不回显原值。
...
```powershell
node --test tools/d040-first-three-batches-reviewer-assignment-harness.test.mjs
```
