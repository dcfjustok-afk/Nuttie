...
`phase` 表示最后一个已经由有效观察证明完成的阶段。每个 effect 发出后保存在 `pendingEffect`，只有匹配当前 `operationId + phase + attempt + protocolVersion` 的 outcome 才能推进。状态、intent、effect 和观察值均被复制并深度冻结，只包含可序列化 plain data。
