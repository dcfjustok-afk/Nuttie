...
> 日期：2026-07-31
>
> 规则：Owner 明确确认并写入治理台账前，任何 Recommended 都不是最终选型，也不得据此初始化工程。
>
...
**Recommended**：所有图片逐次预览；纯文本至少在发送按钮附近持续显示 host/model，并在 host/model 变化后的首次请求重新确认。若 Owner 更重视一致性，可对全部载荷采用逐次预览。
