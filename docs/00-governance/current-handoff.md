...
`project-ops/snapshots/current.json` 已于 `2026-08-13T20:25:12+08:00` 完成人工归并，当前记录 17 项 accepted、14 项 candidate、129 个事件、114 条 Agent 消息、25 个角色和 1 个活跃角色；唯一活跃角色是 PM `root`。最新事件登记 F16 严格 opaque AI response、易失参考草稿、本地 revision 编辑和放弃清理合同；它把输出固定为 `REFERENCE_ONLY / NOT_MEDICAL_ADVICE` 且所有 effect 为零，不授权 UXD-04/UXD-11、D-033/D-053、医疗安全、高风险用途、正式 payload、自动改日记/目标、持久化、网络或 Gate。此前 F01/F02 AI 候选确认、AI 凭据、F04/F05/F06/F08/F10/F11/F12/F13/F14/F15/F17/F18/F21、F20/F23/F24 禁止能力和 F22 平台/语言 Release 合同仍保持原边界。两项 Release 聚合审计当前都明确为 `BLOCKED`：没有正式签名 Release Archive，F20/F23/F24 缺 27 面生产报告；F22 还缺设备族、方向、Mac、Vision availability 四项权威决定和 25 面发布报告。OI-03 已通过原生 choice-ui 记录为当前只有 `iPhone 16 Pro Max / iOS 26.5`、暂无可用 Mac；两路独立审查确认这不授权 iOS 原生工作。D-039 仍保持 PX-2，D-040 仍阻断于 PX-0。
...
8. **OI-02 的原生 Choice UI 宿主尚未就绪。** 2026-08-14 只读审计确认当前任务没有 `ask_choice`，Choice UI MCP 未注册且 `enable_mcp_apps` 未启用；0.1.0 会降级为文字列表，不符合门禁，0.3.0 的协议与浏览器宿主模拟通过但仍未安装。完整证据见 [Choice UI 宿主只读审计](../04-engineering/choice-ui-host-audit-2026-08-14.md)。这不改变 Owner intake、D-039/D-040 或任何实现授权。

当前机器验收基线包含 5 个 Schema、`246` 个受控实例、`31` 条决定、1 份 Owner intake、`129` 个事件、`114` 条消息和 25 个角色；事件/消息 ID 唯一且所有 `responseTo` 可解析。验证器锁定 AI 凭据、F01/F02 AI 候选确认、F16 AI 参考草稿、F04/F05/F06/F08/F10/F11/F12/F13/F14/F15/F17/F18/F21、F20/F23/F24 禁止能力与 F22 平台/语言非生产合同，拒绝把它们冒充 Owner、产品规则、平台形态决定、真实原生调用、正式 Release 目标、生产报告或正式实现授权。`project-ops/reconcile.mjs` 对账 OI-03 精确事实、OI-02 choice-ui 下一题、D-039 PX-2 和 D-040 PX-0 六项授权位。证据矩阵仍为 `66 = 37 confirmed + 24 cross-source + 5 pending`。只读环境审计记录于 [本机 iOS 工具链审计](../04-engineering/local-ios-toolchain-audit-2026-08-06.md)：当前 Windows 主机不可见 Expo CLI、`xcodebuild` 和 CocoaPods；与 Owner 当前无可用 Mac 的事实一致，但不替代未来 Mac 实机审计。
...
当前 Codex host 在 OI-02 前还需要完成 [Choice UI 宿主只读审计](../04-engineering/choice-ui-host-audit-2026-08-14.md) 第 6 节列出的全局插件/feature 授权与新任务工具发现。未获得该授权前，不得安装插件、修改全局 Codex 配置，或用 0.1.0 的文字 fallback 冒充原生选择卡。
