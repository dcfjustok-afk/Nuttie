...
| G2 | 产品基线 | IN_PROGRESS | F01-F24、REQ-F01-F24、AT-F01-F24 集合完整；五项追踪整改最终复验通过；首批 Owner 输入已确认；首个 MVP 增量与后续范围边界三选一卡及跨角色复核包已准备，仍待具名复核人指派、实际复核、Owner 选择与正式冻结 |
...
当前状态：`IN_PROGRESS`。能力、需求和验收主键已经 24/24 对齐，F13 已明确区分 confirmed 的 Burned/近 7 日消耗与 cross-source 的运动/步数。[五项追踪整改最终复验](../05-quality/traceability-review.md) 已通过；首批 Owner 输入已经整批确认。[首个 MVP 增量与后续范围边界决策卡](../02-product/mvp-increment-scope-card.md)已准备 A/B/C 三项互斥选择；[跨角色复核包](../02-product/mvp-increment-scope-review-packet.md)已固定 11 份输入、3 个选项、5 个复核域、12 条跨选项不变量和 P0~P3 标准，但目前仅为 `PACKET_READY`，具名复核人尚未指派、实际复核尚未开始。推荐 A 不等于 Owner 已选择，范围尚未冻结，G2 继续 `IN_PROGRESS`。该选择不再等待 D-052/D-053 关闭本地自用、无 USDA 境外再分发、无第三方 AI 的开发路径。
