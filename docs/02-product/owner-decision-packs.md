...
| D-040 | 首启资料与目标 | A 最小资料 + 可解释公式候选 + 用户确认；B 只手工；C 强制完整问卷 | 未到选择阶段 | 20 个独立决定轴已分配 ID；前三批十三卡复核包和健康交接包就绪；D-063 来源、D-070 输入、D-071 显示与 D-072 硬停止记录卡内部成稿，但前置接受、健康数值边界与文案、具名复核和采用证据仍未关闭 |
