...
- 当前第 1 批由 PM 在主 Codex 聊天中通过原生 `request_user_input` 选择卡逐题询问，每轮只提交一个稳定 D 编号；Owner 点击选项后，PM 才进入下一题。不要求 Owner输入字母，也不允许用网页表单或复制模板替代。
...
新增全局候选：`D-037` 包管理器、`D-047` Apple 分发身份、`D-048` 设备与方向、`D-049` TestFlight 测试层级、`D-050` 隐私/支持页面发布形态、`D-051` 长期分发渠道与地区、`D-052` USDA 境外再分发、`D-053` 第三方 AI Provider 数据用途准入。
...
| D-045 | 最近使用/收藏 | A 最近首版、收藏后续；B 同时首版；C 都后续 | A |
...
当前下一张卡是 `OI-03 设备条件`，用于确认是否有可用 Mac 和 iPhone。随后关闭 OI-02：SKU 因当前不使用 App Store Connect 可记 `N/A`，Bundle ID 最迟在首次自用真机签名配置前用具体候选卡确认。事实输入没有值时可选 `N/A`、`无`、`申请中`、`尚未创建` 或 `UNKNOWN`。

逐题点击期间，所有选择只作为待回读输入。首批完成后，项目经理必须回传一份规范化解析：逐项列出状态是 `ACCEPTED`、`CANDIDATE + SPIKE_AUTHORIZED` 还是 `DEFERRED`，并复述 OI 事实值，再通过原生选择卡请求整批最终确认。Owner 点击确认前不追加 `DECISION_ACCEPTED`。D-032 的 A/B 只有“隔离 Spike 授权”含义，不能直接记为最终接受；后续验证失败必须提交新决定，不允许团队自动切换选项。当前线程若处于 Default 模式，必须先切换到 Plan 模式以启用该工具。
