...
| D-040 | 首启资料与目标 | A 最小资料 + 可解释公式候选 + 用户确认；B 只手工；C 强制完整问卷 | 未到选择阶段 | 20 个独立决定轴已分配 ID；十三卡、健康与四张宏量轴卡复核包就绪，宏量包输入待冻结；前置接受、健康数值边界与文案、具名复核和采用证据仍未关闭 |
