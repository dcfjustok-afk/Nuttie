...
| D-040 | 首启资料与目标 | A 最小资料 + 可解释公式候选 + 用户确认；B 只手工；C 强制完整问卷 | 未到选择阶段 | 20 个独立决定轴已分配 ID；前三批十三张内部卡片完成四域自审，动态模型证据与独立复核待完成 |
...
OI-02 已确认 Bundle ID 尚未创建，具体值为空，App ID 与 App Store Connect record 均未创建，SKU=`N/A`；具体 Bundle ID 最迟在首次自用真机签名配置前另行确认。OI-03 已确认当前只有 `iPhone 16 Pro Max / iOS 26.5`、暂无可用 Mac。D-039 已由 Owner 选择 A 并完成 PX-4；D-040 仍只是计划中的宿主原生队列占位，前三批十三张内部卡片已完成四域自审，当前处于 `PX-0_INPUT_GAP / FIRST_THREE_BATCHES_INDEPENDENT_REVIEW_REQUIRED`，动态模型证据和独立复核未完成，不得提前展示。
