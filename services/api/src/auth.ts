...
import { RepositoryError, type Repository, type SessionRecord, type UserRecord } from "./store.js";

const PASSWORD_PREFIX = "scrypt-v1";

function deriveScrypt(password: string, salt: Buffer, length: number): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    scryptCallback(password, salt, length, { N: 16_384, r: 8, p: 1, maxmem: 64 * 1024 * 1024 }, (error, derived) => {
...
function issueAccessToken(userId: string, sessionId: string, secret: string, ttlSeconds: number, nowMs: number): string {
  const header = base64url(JSON.stringify({ alg: "HS256", typ: "NUTTIE" }));
  const now = Math.floor(nowMs / 1000);
  const payload: AccessClaims = { sub: userId, sid: sessionId, iat: now, exp: now + ttlSeconds, typ: "access" };
  const encodedPayload = base64url(JSON.stringify(payload));
  const data = `${header}.${encodedPayload}`;
  return `${data}.${tokenSignature(data, secret)}`;
}

function decodeAccessToken(token: string, secret: string, nowMs: number): AccessClaims {
  const parts = token.split(".");
  if (parts.length !== 3) throw new AuthError("UNAUTHORIZED", "invalid access token");
  const header = parts[0] as string;
  const payload = parts[1] as string;
  const signature = parts[2] as string;
  const expected = fromBase64url(tokenSignature(`${header}.${payload}`, secret));
  const actual = fromBase64url(signature);
  if (expected.length !== actual.length || !timingSafeEqual(expected, actual)) throw new AuthError("UNAUTHORIZED", "invalid access token");
  let decoded: unknown;
  try {
  ...
  if (!decoded || typeof decoded !== "object") throw new AuthError("UNAUTHORIZED", "invalid access token");
  const claims = decoded as Partial<AccessClaims>;
  const now = Math.floor(nowMs / 1000);
  if (claims.typ !== "access" || typeof claims.sub !== "string" || typeof claims.sid !== "string" || typeof claims.exp !== "number" || claims.exp <= now) {
...
export class AuthService {
  ...
  async register(input: RegisterInput): Promise<AuthResponse> {
    ...
    try {
      ...
      if (error instanceof RepositoryError && error.code === "EMAIL_TAKEN") throw new AuthError("EMAIL_TAKEN", error.message, 409);
      throw error;
    }
  ...
  async refresh(refreshToken: string | undefined): Promise<AuthResponse> {
    if (!refreshToken) throw new AuthError("INVALID_REFRESH_TOKEN", "refresh token is required", 401);
    const session = await this.repository.findSessionByTokenHash(tokenHash(refreshToken));
    if (!session || session.revokedAt || Date.parse(session.expiresAt) <= this.now()) {
      throw new AuthError("INVALID_REFRESH_TOKEN", "refresh token is invalid or expired", 401);
    }
    const user = await this.repository.findUserById(session.userId);
    if (!user) throw new AuthError("INVALID_REFRESH_TOKEN", "session user no longer exists", 401);
    await this.repository.revokeSession(session.id, new Date(this.now()).toISOString());
    return this.issueSession(user);
  }

  async logout(input: { refreshToken?: string; accessToken?: string }): Promise<void> {
    let sessionId: string | undefined;
    if (input.refreshToken) {
      const session = await this.repository.findSessionByTokenHash(tokenHash(input.refreshToken));
      sessionId = session?.id;
    }
    if (!sessionId && input.accessToken) {
      try {
        sessionId = decodeAccessToken(input.accessToken, this.secret, this.now()).sid;
      } catch {
        // Logout is idempotent even when an already expired access token is supplied.
      }
    }
    if (sessionId) await this.repository.revokeSession(sessionId, new Date(this.now()).toISOString());
  }

  async authenticate(accessToken: string | undefined): Promise<User> {
    if (!accessToken) throw new AuthError("UNAUTHORIZED", "authorization is required", 401);
    ...
    if (!session || session.revokedAt || Date.parse(session.expiresAt) <= this.now() || session.userId !== claims.sub) {
      throw new AuthError("UNAUTHORIZED", "session is no longer active", 401);
    }
    const user = await this.repository.findUserById(claims.sub);
    if (!user) throw new AuthError("UNAUTHORIZED", "account no longer exists", 401);
    return asUser(user);
  }
  ...
  private async issueSession(user: UserRecord): Promise<AuthResponse> {
    ...
    const response: AuthResponse = {
      user: asUser(user),
      accessToken: issueAccessToken(user.id, session.id, this.secret, this.accessTtl, now),
      refreshToken,
      tokenType: "Bearer",
