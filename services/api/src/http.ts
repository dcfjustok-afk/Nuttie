...
import { hashMutationFingerprint, RepositoryError, type Repository } from "./store.js";
...
class HttpError extends Error {
  ...
  constructor(status: number, code: ApiErrorCode | (string & {}), message: string, details?: HttpErrorDetails) {
    super(message);
    this.name = "HttpError";
    this.status = status;
    this.code = code;
...
function applyCors(request: IncomingMessage, response: ServerResponse, config: RuntimeConfig): void {
...
async function readJson(request: IncomingMessage): Promise<unknown> {
  ...
  if (contentType !== "application/json") throw new HttpError(415, "BAD_REQUEST", "content-type must be application/json");
  const chunks: Buffer[] = [];
  let total = 0;
  for await (const chunk of request) {
    const buffer = Buffer.isBuffer(chunk) ? chunk : Buffer.from(chunk);
    total += buffer.length;
    if (total > MAX_BODY_BYTES) throw new HttpError(413, "BAD_REQUEST", "request body is too large");
    chunks.push(buffer);
  }
  if (total === 0) return {};
  try {
...
function browserSession(request: IncomingMessage, config: RuntimeConfig): boolean {
  const platform = request.headers["x-client-platform"]?.toString().trim().toLowerCase();
  if (platform === "web" || platform === "browser") return true;
  const origin = request.headers.origin;
  return typeof origin === "string" && config.corsOrigins.includes(origin);
}
...
function authPayload(
  ...
  if (browserSession(request, config)) {
...
function errorResponse(error: unknown, requestId: string): { status: number; body: unknown } {
  if (error instanceof HttpError) {
    return { status: error.status, body: apiError(error.code, error.message, error.details, requestId) };
  }
  if (error instanceof ZodError) {
    return { status: 400, body: apiError("VALIDATION_ERROR", "request validation failed", errorDetails(error), requestId) };
  }
  if (error instanceof AuthError) {
    return { status: error.status, body: apiError(error.code, error.message, error.details, requestId) };
  }
  if (error instanceof RepositoryError) {
    const status = error.code === "EMAIL_TAKEN" ? 409 : error.code === "BAD_CURSOR" ? 400 : 503;
    const code = error.code === "BAD_CURSOR" ? "BAD_REQUEST" : error.code;
    return { status, body: apiError(code, error.message, error.details, requestId) };
  }
  console.error(JSON.stringify({ level: "error", requestId, message: error instanceof Error ? error.message : String(error) }));
  return { status: 500, body: apiError("INTERNAL_ERROR", "an unexpected error occurred", undefined, requestId) };
}

function isRoute(method: string | undefined, pathname: string, expectedMethod: string, expectedPath: string): boolean {
  return method === expectedMethod && pathname === expectedPath;
}

async function route(request: IncomingMessage, response: ServerResponse, context: ApiContext): Promise<void> {
  ...
  if (isRoute(request.method, url.pathname, "GET", "/health") || isRoute(request.method, url.pathname, "GET", "/api/v1/health")) {
    sendJson(response, 200, { status: "ok", service: "nuttie-api", version: "v1" });
    return;
  }
  if (isRoute(request.method, url.pathname, "GET", "/ready") || isRoute(request.method, url.pathname, "GET", "/api/v1/ready")) {
  ...
  if (isRoute(request.method, url.pathname, "POST", "/api/v1/auth/register")) {
    ...
    if (result.refreshToken) response.setHeader("set-cookie", refreshCookie(result.refreshToken, context.config));
    sendJson(response, 201, { data: authPayload(request, context.config, result) });
    return;
  }
  if (isRoute(request.method, url.pathname, "POST", "/api/v1/auth/login")) {
    ...
    if (result.refreshToken) response.setHeader("set-cookie", refreshCookie(result.refreshToken, context.config));
    sendJson(response, 200, { data: authPayload(request, context.config, result) });
    return;
  }
  if (isRoute(request.method, url.pathname, "POST", "/api/v1/auth/refresh")) {
    ...
    const result = await context.auth.refresh(input.refreshToken ?? parseCookie(request, COOKIE_NAME));
    if (result.refreshToken) response.setHeader("set-cookie", refreshCookie(result.refreshToken, context.config));
    sendJson(response, 200, { data: authPayload(request, context.config, result) });
    return;
  }
  ...
  if (isRoute(request.method, url.pathname, "GET", "/api/v1/sync")) {
    ...
    const query = z
      .object({ cursor: z.string().optional(), limit: z.coerce.number().int().positive().max(500).default(100) })
      .parse({ cursor: url.searchParams.get("cursor") ?? undefined, limit: url.searchParams.get("limit") ?? undefined });
    const sync = await context.repository.listChanges(user.id, query.cursor, query.limit);
  ...
  if (isRoute(request.method, url.pathname, "POST", "/api/v1/mutations")) {
    ...
    const committed = await context.repository.commitMutation(user.id, mutation, hashMutationFingerprint(mutation));
    if (committed.status === "idempotency-conflict") {
      throw new HttpError(409, "IDEMPOTENCY_CONFLICT", "clientMutationId was already used with different content", {
        clientMutationId: mutation.clientMutationId,
      });
    }
    if (committed.status === "revision-conflict") {
      throw new HttpError(409, "REVISION_CONFLICT", "record revision is stale", {
    ...
    if (committed.status === "duplicate-record") {
      throw new HttpError(409, "DUPLICATE_RECORD", "record already exists", { record: committed.record });
    }
    ...
    sendJson(response, committed.status === "committed" ? 201 : 200, { data: result });
    return;
  }
