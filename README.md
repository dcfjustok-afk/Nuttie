...
Nuttie 是一款面向自己与朋友使用的本地优先跨平台产品，首期覆盖 iOS、Android 与 React Native Web/mobile H5。产品以公开可验证的“自律茄子”能力为对标基线，使用 Expo + React Native 构建；记录先写入各端本地缓存和离线队列，登录后通过最小 API 与 PostgreSQL 跨设备同步。除用户主动发起的 AI 请求外，AI key 与原始 AI 载荷不进入云同步。

## 当前阶段

- 方向修订：2026-08-29（D-073：iOS、Android、RN Web/mobile H5 + 账号云同步）
- 历史快照：2026-08-27（ProjectOps validator/snapshot 尚待迁移）
- D-036 报告合同：本地 fail-closed validator 已完成 20 项定向测试，正式覆盖下限为 36 个兼容单元、324 条 raw attempt、108 条离线结果和 39 条原生边界结果；工件仍未提交，实际 run/report/native evidence、联网、复核、Owner、B05 和实现授权均为 0/false
- D-053 报告合同：Provider 证据与 App Privacy report 机器合同及本地 fail-closed validator 已完成 19 项定向测试，正式矩阵固定为 3 个 Provider × 5 类 payload = 15 个 profile、150 项十维 assessment 和 45 项 A-B-C comparison；工件仍未提交，真实 OI-07、Provider、来源/快照、映射、具名签署、独立复核、Owner、准入、B05、联网和实现均为 0/false
- 阶段：Phase 0，跨平台产品、体验、架构基线形成中
- `G0 项目立项`：PASS
...
- 手机优先但 adaptive：覆盖 320px 窄屏、375-600px 手机、横屏、601-1023px 平板和 1024px+ 桌面；大屏使用最大内容宽度与更高信息密度，不无限拉伸表单或用视口宽度缩放字号。
- PostgreSQL 是已登录用户的同步权威；每个客户端保留本地缓存和可重放 outbox。mutation 使用 `clientMutationId`、`deviceId`、`entityId`、`baseRevision` 和单调 `revision`，冲突必须显式呈现。
- Web 使用同源 `/api` 与安全 httpOnly refresh cookie；native 使用平台安全存储；Web 的 IndexedDB/WebCrypto/XSS/CSRF 风险不能套用 SQLCipher/Keychain 假设。
- 首期服务只提供版本化 REST 和 readiness；Redis、WebSocket、对象存储、运营后台、远程 Push 和服务端 AI 代理后置。部署可参考 `D:\github\Royal-Flush` 的 Nginx 同源反代和私有 API/PostgreSQL 拓扑，但本次 push 不等于生产部署。

## 硬边界

- 仅 `AITransport` 可在用户主动操作后访问其配置的 HTTPS AI 服务。
- 仅建设 D-073 允许的最小账号/同步 API 与 PostgreSQL；不引入超出范围的对象存储、遥测、广告、远程配置、远程 Push、崩溃上传、CloudKit 或 EAS OTA。
- AI Base URL、model 与 key 由每位使用者自行配置；凭据不得打包进 IPA。
- native 业务数据库使用 SQLCipher，数据库密钥存入 Keychain；Web 采用独立的浏览器存储和安全威胁模型。
- 数据恢复采用用户主动执行的加密文件导出/导入，默认排除 iCloud。
- iOS native 最低版本为 17；Android/Web 最低支持矩阵在 G4 前冻结；所有首发平台仅提供简体中文（`zh-Hans`）。

## 项目入口

- [跨平台方向基线（D-073）](docs/02-product/platform-reframe.md)
- [跨平台云同步 ADR](docs/04-engineering/adr/0006-cross-platform-cloud-sync.md)
...
- [D-036 Provider/原生兼容 Spike 协议](docs/04-engineering/testing/d036-provider-native-compatibility-spike-protocol.md)
- [D-036 Provider/原生兼容报告机器合同](docs/04-engineering/testing/d036-provider-native-compatibility-report-contract.md)
- [D-036 Provider/原生兼容报告本地校验合同](docs/04-engineering/testing/d036-provider-native-compatibility-report-harness.md)
- [D-053 AI Provider 用途准入选择卡](docs/03-design/d053-ai-provider-use-admission-card-spec.md)
- [D-053 Provider 证据与 App Privacy 映射协议](docs/04-engineering/testing/d053-provider-evidence-app-privacy-protocol.md)
- [D-053 Provider 证据与 App Privacy 报告机器合同](docs/04-engineering/testing/d053-provider-evidence-app-privacy-report-contract.md)
- [D-053 Provider 证据与 App Privacy 报告本地校验合同](docs/04-engineering/testing/d053-provider-evidence-app-privacy-report-harness.md)
...
- [D-040 D-068/D-069 非诊断边界选择卡规格](docs/03-design/d040-non-diagnostic-boundary-card-spec.md)
- [D-040 D-068/D-069 非诊断边界独立复核包](docs/03-design/d040-non-diagnostic-boundary-independent-review-packet.md)
- [D-040 D-068/D-069 非诊断边界独立复核回执机器合同](docs/04-engineering/testing/d040-non-diagnostic-boundary-independent-review-record-contract.md)
- [D-040 D-068/D-069 非诊断边界独立复核回执本地校验合同](docs/04-engineering/testing/d040-non-diagnostic-boundary-independent-review-record-harness.md)
- [D-040 D-068/D-069 非诊断边界独立复核人指派机器合同](docs/04-engineering/testing/d040-non-diagnostic-boundary-reviewer-assignment-contract.md)
- [D-040 D-068/D-069 非诊断边界独立复核人指派本地校验合同](docs/04-engineering/testing/d040-non-diagnostic-boundary-reviewer-assignment-harness.md)
- [D-040 D-068/D-069 非诊断边界正式复核交接清单](docs/03-design/d040-non-diagnostic-boundary-review-handoff-checklist.md)
- [D-040 D-068/D-069 非诊断边界复核启动缺口登记表](docs/03-design/d040-non-diagnostic-boundary-review-start-gap-register.md)
- [D-040 D-068/D-069 非诊断边界正式指派记录空白模板](docs/04-engineering/testing/d040-non-diagnostic-boundary-formal-assignment-record-template.md)
- [D-040 D-068/D-069 非诊断边界指派授权预检清单](docs/03-design/d040-non-diagnostic-boundary-assignment-authorization-preflight-checklist.md)
- [D-040 D-068/D-069 非诊断边界联络授权记录合同](docs/04-engineering/testing/d040-non-diagnostic-boundary-contact-authorization-record-contract.md)
- [D-040 D-068/D-069 非诊断边界候选名单记录合同](docs/04-engineering/testing/d040-non-diagnostic-boundary-reviewer-candidate-roster-contract.md)
- [D-040 D-068/D-069 非诊断边界材料包记录合同](docs/04-engineering/testing/d040-non-diagnostic-boundary-review-material-packet-record-contract.md)
