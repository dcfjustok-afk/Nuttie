...
export default function TrendsScreen() {
  ...
  const todayValue =
    metric === "摄入"
      ? meals.reduce((sum, record) => sum + (record.energyKcal ?? 0), 0)
      : metric === "饮水"
  ...
  const values = useMemo(
    () =>
      sampleDays.map((_, index) =>
        index === sampleDays.length - 1 ? todayValue : 0,
      ),
    [todayValue],
  );
  const max = Math.max(...values, metric === "体重" ? 1 : 100);
  const summary = todayValue
    ? `今天${metric} ${metric === "体重" ? `${todayValue.toFixed(1)} kg` : `${Math.round(todayValue)}${metric === "摄入" ? " kcal" : " ml"}`}`
    : `今天还没有${metric}记录`;
  return (
    <Screen>
      ...
      <View style={[styles.switcher, { backgroundColor: colors.surfaceMuted }]}>
        {metricOptions.map((item) => (
          ...
          >
            <Text
              style={[
                styles.switchText,
                { color: metric === item ? colors.ink : colors.inkMuted },
              ]}
            >
              {item}
            </Text>
      ...
      <View style={[styles.layout, wide && styles.layoutWide]}>
        ...
        >
          <View style={styles.panelHeading}>
            <View>
              ...
              <Text style={[styles.panelCaption, { color: colors.inkMuted }]}>
                按已保存的本地数据展示
              </Text>
            </View>
            <Text style={[styles.unit, { color: colors.inkMuted }]}>
              {metric === "摄入" ? "kcal" : metric === "饮水" ? "ml" : "kg"}
            </Text>
          </View>
          <View
            accessibilityRole="image"
            accessibilityLabel={`${metric}七日图，${summary}`}
            style={styles.chart}
          >
            {values.map((value, index) => (
              <View key={sampleDays[index]} style={styles.barColumn}>
                <View
                ...
                <Text style={[styles.day, { color: colors.inkMuted }]}>
                  {sampleDays[index]}
                </Text>
        ...
        >
          ...
          <Text style={[styles.detail, { color: colors.inkMuted }]}>
            只有今天有记录时，其他日期会保持空白。空白代表没有数据，不代表没有发生。
          </Text>
          <View style={[styles.legend, { borderTopColor: colors.border }]}>
            <View
              style={[styles.legendDot, { backgroundColor: colors.sprout }]}
            />
      ...
      >
        <Text style={[styles.noteTitle, { color: colors.ink }]}>关于趋势</Text>
        <Text style={[styles.noteBody, { color: colors.inkMuted }]}>
          趋势页帮助你回看自己的记录，不提供医疗判断，也不会替你设定目标。
        </Text>
      </View>
    </Screen>
  );
