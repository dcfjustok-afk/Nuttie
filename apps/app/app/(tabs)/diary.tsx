...
export default function DiaryScreen() {
  ...
  const wide = sizeClass === "expanded" || sizeClass === "wide";
  ...
  const energy = meals.reduce(
    (sum, record) => sum + (record.energyKcal ?? 0),
    0,
  );
  const protein = meals.reduce(
    (sum, record) => sum + (record.proteinG ?? 0),
    0,
  );
  ...
  const progress = useMemo(
    () =>
      Math.min(
        1,
        (meals.length + (water.length ? 1 : 0) + (weight ? 1 : 0)) / 5,
      ),
    [meals.length, water.length, weight],
  );
  const growthState = getGrowthState(
    progress,
    records.some((record) => record.syncStatus === "pending"),
  );
    ...
    <Screen>
      <View style={styles.topLine}>
        <View style={styles.dateBlock}>
          <Text style={[styles.kicker, { color: colors.inkMuted }]}>今天</Text>
          <Text style={[styles.date, { color: colors.ink }]}>{dateLabel}</Text>
        </View>
        <SyncBadge />
      </View>
      <View
      ...
      >
        ...
        <View style={styles.heroCopy}>
          ...
          <Text style={[styles.heroBody, { color: colors.inkMuted }]}>
            记录是事实，不是评分。目标还没设置也没关系，先从一件小事开始。
          </Text>
          <View style={styles.heroMeta}>
            <View
              style={[styles.metaDot, { backgroundColor: colors.sprout }]}
            />
      ...
      <View style={styles.sectionHeading}>
        <View>
          <Text style={[styles.heading, { color: colors.ink }]}>今日账本</Text>
        ...
        >
          <Icon name="add" size={18} color={colors.inverse} />
          <Text style={[styles.addText, { color: colors.inverse }]}>新增</Text>
        </Pressable>
      </View>
      <View style={[styles.metricGrid, wide && styles.metricGridWide]}>
        <View style={wide && styles.metricCellWide}>
          <MetricBand
            label="今日摄入"
            value={energy ? String(Math.round(energy)) : "未记录"}
            unit={energy ? "kcal" : undefined}
            hint={energy ? "来自已保存餐食" : "添加餐食后显示"}
            tone="chestnut"
          />
        </View>
        <View style={wide && styles.metricCellWide}>
          <MetricBand
            label="蛋白质"
            value={protein ? String(Math.round(protein)) : "未提供"}
            unit={protein ? "g" : undefined}
            hint="缺失值不会按 0 处理"
            tone="sprout"
          />
        </View>
        <View style={wide && styles.metricCellWide}>
          <MetricBand
            label="饮水"
            value={
              water.reduce((sum, record) => sum + (record.amount ?? 0), 0)
                ? String(
                    Math.round(
                      water.reduce(
            ...
            unit={water.length ? "ml" : undefined}
            hint="手工记录"
            tone="sky"
          />
        </View>
        <View style={wide && styles.metricCellWide}>
          <MetricBand
            label="体重"
            value={
              weight?.amount !== undefined ? String(weight.amount) : "未记录"
            }
            unit={weight?.unit}
            hint="仅作个人记录"
            tone="amber"
      ...
      >
        <View style={styles.macroHeader}>
          <Text style={[styles.macroTitle, { color: colors.ink }]}>
            宏量快照
          </Text>
          <Text style={[styles.macroHint, { color: colors.inkMuted }]}>
            今日已有数据
          </Text>
        </View>
        <View style={styles.macroValues}>
          <Macro label="碳水" value={carbs} color={colors.chestnut} />
          <Macro label="蛋白质" value={protein} color={colors.sprout} />
          <Macro label="脂肪" value={fat} color={colors.skyDark} />
        </View>
      </View>
      <View style={styles.sectionHeading}>
        <View>
          <Text style={[styles.heading, { color: colors.ink }]}>最近记录</Text>
          <Text style={[styles.caption, { color: colors.inkMuted }]}>
            {records.length
              ? `${records.length} 条记录 · 新记录会排在最前`
              : "还没有记录"}
          </Text>
        </View>
      </View>
      <View style={styles.recordList}>
        ...
        {!records.length && <EmptyRecords onAdd={() => setSheetOpen(true)} />}
      </View>
...
function Macro({
  ...
  return (
    <View style={styles.macro}>
      <View style={[styles.macroMarker, { backgroundColor: color }]} />
      <Text style={[styles.macroLabel, { color: colors.inkMuted }]}>
        {label}
      </Text>
      <Text style={[styles.macroValue, { color: colors.ink }]}>
        {value ? `${Math.round(value)} g` : "未提供"}
      </Text>
    </View>
  );
}
function EmptyRecords({ onAdd }: { onAdd: () => void }) {
  ...
  return (
    ...
    >
      <Icon name="leaf" size={24} color={colors.sprout} />
      <Text style={[styles.emptyTitle, { color: colors.ink }]}>
        从一件小事开始
      </Text>
      <Text style={[styles.emptyBody, { color: colors.inkMuted }]}>
        餐食、饮水或体重都可以。保存后你随时能补充细节。
      </Text>
      <Pressable
        accessibilityRole="button"
        onPress={onAdd}
        style={[styles.emptyAction, { borderColor: colors.chestnut }]}
      >
        <Text style={[styles.emptyActionText, { color: colors.chestnut }]}>
          新增第一条
        </Text>
      </Pressable>
    </View>
...
const styles = StyleSheet.create({
  ...
  emptyActionText: { ...typeScale.label },
});
