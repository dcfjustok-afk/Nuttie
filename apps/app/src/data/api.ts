...
async function request<T>(
  ...
  if (!response.ok) {
    ...
    throw new ApiRequestError(
      ...
      apiError?.message ?? `请求失败（${response.status}）`,
      apiError?.details,
    );
  }
  return (
    "data" in payload && payload.data !== undefined ? payload.data : payload
  ) as T;
}
...
export async function login(email: string, password: string): Promise<Session> {
  return authenticated(
    await request<Omit<Session, "mode">>("/v1/auth/login", {
      method: "POST",
...
export async function register(
  ...
  return authenticated(
    await request<Omit<Session, "mode">>("/v1/auth/register", {
      method: "POST",
...
export async function refresh(refreshToken?: string): Promise<Session> {
  return authenticated(
    await request<Omit<Session, "mode">>("/v1/auth/refresh", {
      method: "POST",
...
export async function pullSync(
  accessToken: string,
  cursor?: string,
): Promise<{ cursor: string; records: LocalRecord[] }> {
  return request<{ cursor: string; records: LocalRecord[] }>(
...
export async function pushMutation(
  ...
  // Apply the same structural privacy gate as the API before a queued
  // mutation leaves the device.
