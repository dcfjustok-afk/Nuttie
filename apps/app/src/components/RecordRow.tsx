  ...
  return (
    ...
    >
      ...
      <View style={styles.copy}>
        ...
        <Text
          style={[styles.subtitle, { color: colors.inkMuted }]}
          numberOfLines={1}
        >
          {record.subtitle}
        </Text>
      ...
      <View style={styles.amount}>
        {record.amount !== undefined && (
          <Text style={[styles.amountValue, { color: colors.ink }]}>
            {record.amount}
            <Text style={[styles.amountUnit, { color: colors.inkMuted }]}>
              {" "}
              {record.unit}
            </Text>
          </Text>
        ...
        {record.energyKcal !== undefined && (
          <Text style={[styles.energy, { color: colors.inkMuted }]}>
            {record.energyKcal} kcal
          </Text>
        )}
        {record.syncStatus === "pending" && (
          <Text style={[styles.sync, { color: colors.amber }]}>待同步</Text>
        )}
        {record.syncStatus === "conflict" && (
          <Text style={[styles.sync, { color: colors.danger }]}>需处理</Text>
        )}
      </View>
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  copy: { flex: 1, minWidth: 0, gap: componentTokens.recordRow.copyGap },
  title: { ...typeScale.body, fontWeight: "700" },
  subtitle: { ...typeScale.caption },
  amount: { alignItems: "flex-end", gap: componentTokens.recordRow.copyGap },
