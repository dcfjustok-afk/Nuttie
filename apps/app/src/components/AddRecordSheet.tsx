...
export function AddRecordSheet({ visible, onClose }: { visible: boolean; onClose: () => void }) {
  ...
  function selectKind(next: RecordKind) {
    setKind(next);
    if (next === "meal") { setTitle("午餐"); setUnit("份"); }
    if (next === "water") { setTitle("饮水"); setUnit("ml"); }
    if (next === "weight") { setTitle("体重记录"); setUnit("kg"); }
  }

  async function submit() {
    const numericValues = [amount, energy, protein, carbs, fat].filter(Boolean).map(Number);
    ...
    try {
      await addRecord({
        kind,
        title: title.trim() || (kind === "meal" ? "餐食" : kind === "water" ? "饮水" : "体重记录"),
        subtitle: "手工记录 · 刚刚",
      ...
      setAmount(""); setEnergy(""); setProtein(""); setCarbs(""); setFat("");
      onClose();
    } catch (cause) {
  ...
  return (
    <Modal visible={visible} animationType="slide" transparent onRequestClose={onClose}>
      <View style={[styles.backdrop, large && styles.backdropLarge]}>
        <Pressable accessibilityLabel="关闭新增记录" accessibilityRole="button" onPress={onClose} style={StyleSheet.absoluteFill} />
        <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : undefined} style={styles.keyboard}>
          <View style={[styles.sheet, large && styles.sheetLarge, { backgroundColor: colors.surface, paddingBottom: Math.max(insets.bottom + spacing.lg, spacing.lg) }]}>
...
function Field({ label, value, onChangeText, placeholder, keyboardType, colors }: { label: string; value: string; onChangeText: (value: string) => void; placeholder: string; keyboardType?: "decimal-pad"; colors: ReturnType<typeof import("@nuttie/design-tokens").getSemanticColors> }) {
  return <View style={styles.field}><Text style={[styles.fieldLabel, { color: colors.ink }]}>{label}</Text><TextInput accessibilityLabel={label} value={value} onChangeText={onChangeText} placeholder={placeholder} placeholderTextColor={colors.inkSubtle} keyboardType={keyboardType} style={[styles.input, { color: colors.ink, backgroundColor: colors.surfaceMuted, borderColor: colors.border }]} /></View>;
}
...
const styles = StyleSheet.create({
  ...
  close: { width: dimensions.minTouch, height: dimensions.minTouch, alignItems: "center", justifyContent: "center", marginTop: -8, marginRight: -8 },
  segmented: { minHeight: 48, borderRadius: radii.compact, padding: 4, flexDirection: "row", gap: 4 },
  segment: { minHeight: dimensions.minTouch, flex: 1, borderRadius: 8, borderWidth: StyleSheet.hairlineWidth, borderColor: "transparent", flexDirection: "row", alignItems: "center", justifyContent: "center", gap: spacing.xs },
  ...
  save: { minHeight: dimensions.control, borderRadius: radii.compact, alignItems: "center", justifyContent: "center", paddingHorizontal: spacing.lg },
